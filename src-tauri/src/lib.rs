...
// Auth nonce: a random token generated by the frontend when it opens the browser
// for OAuth login. Deep-link callbacks are only accepted while a nonce is pending
// (within a 5-minute window), preventing rogue vocalype:// URL injection attacks.
static PENDING_AUTH_NONCE: std::sync::Mutex<Option<std::time::Instant>> =
                ...
                if raw_url.contains("auth-callback") {
                    if let Ok(url) = url::Url::parse(raw_url) {
                        if let Some((_, token)) = url.query_pairs().find(|(k, _)| k == "token") {
                            ...
                            // Verify that the app actually initiated a login flow recently.
                            // This prevents rogue vocalype:// URLs from injecting a session.
                            let nonce_valid = PENDING_AUTH_NONCE.lock().ok().map_or(false, |mut guard| {
                                if let Some(started_at) = *guard {
                                    if started_at.elapsed() < std::time::Duration::from_secs(300) {
                                        *guard = None; // consume the nonce
                                        return true;
                                    }
                                }
                                *guard = None; // clear expired nonce
                                false
                            });
                            if !nonce_valid {
                                log::warn!("Deep link auth rejected: no active login flow (possible CSRF attempt)");
                                show_main_window(app);
                                return;
                            }
                    ...
                    if url.scheme() == "vocalype" {
                        ...
                        if host == "auth-callback" {
                            ...
                            if let Some(token) = token_opt {
                                // Only accept deep-link auth when a login flow was started recently.
                                let nonce_valid = PENDING_AUTH_NONCE.lock().ok().map_or(false, |mut guard| {
                                    if let Some(started_at) = *guard {
                                        if started_at.elapsed() < std::time::Duration::from_secs(300) {
                                            *guard = None;
