"""add_benchmark_observation.py â€” V7 Phase 1 Manual Benchmark Recorder.
...
# ---------------------------------------------------------------------------
# Priority metrics â€” used for validation warnings
# ---------------------------------------------------------------------------
...
def _validate(args: argparse.Namespace) -> list[str]:
    ...
    if args.value < 0:
        warnings.append(f"value {args.value} is negative â€” verify this is intentional.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="V7 Manual Benchmark Recorder â€” append one observation.",
...
Examples:
  ...
  python internal/brain/scripts/add_benchmark_observation.py \\
    ...
    if warnings:
        print(f"\n  {len(warnings)} warning(s) above â€” observation was still recorded.")
    print(divider)
    print("Written: internal/brain/data/benchmark_observations.jsonl")
