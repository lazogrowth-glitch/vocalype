...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit('Usage: python vocalype-brain/scripts/retrieve_context.py "license activation problem"')

    query = " ".join(sys.argv[1:]).strip()
    if not INDEX_PATH.exists() or not INDEX_PATH.read_text(encoding="utf-8").strip():
        print("Memory index not found. Run: python vocalype-brain/scripts/index_memory.py")
        return
