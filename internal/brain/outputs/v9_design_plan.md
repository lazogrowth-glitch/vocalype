# Vocalype Brain â€” V9 Design Plan
# Growth / Distribution Loop â€” Content Performance Tracker
...
performed, and what was learned. Over time, V9 surfaces which content attributes â€” platform,
format, hook style, niche, CTA â€” correlate with downstream business outcomes tracked in V8.
...
| Record performance metrics 24â€“72h after posting | Views, likes, comments, saves, shares, profile visits, website clicks |
| Record lesson learned and next content action | Structured free-text fields |
| Surface which content attributes drive profile visits and website clicks | `review_content_performance.py` â†’ `content_report.md` |
| Generate a ranked content backlog by estimated leverage | `distribution_backlog.md` |
| Connect content performance to V8 business outcomes (downloads, signups) | V9 Phase 2 â€” after baseline data exists |

### V9 phases

| Phase | Description | Gate to enter |
|---|---|---|
| Phase 1 â€” Manual content log | Founder records each post. Brain generates weekly content report. | Now â€” no gate |
| Phase 2 â€” V8 correlation | Brain joins V9 content weeks to V8 business weeks. Asks: did content drive downloads? | â‰¥10 content observations across â‰¥2 platforms |
| Phase 3 â€” Experiment comparison | Before/after tracking for format/hook/niche experiments. | â‰¥4 weeks of V9 + V8 joint data |
| Phase 4 â€” Automation of proven channels | Only automates what Phase 3 proves works. | Founder approval required, separate design |

**Phase 4 is not part of this design. It is named only so Phase 1â€“3 have a clear destination.**
...
|---|---|
| Auto-post to any platform | V9 Phase 1â€“3 are manual only. Automation is Phase 4 â€” separate design |
...
| Recommend paid advertising | No CAC/LTV baseline exists â€” paid ads cannot be evaluated |
| Generate content on behalf of the founder | V9 tracks experiments; the founder creates content |
| Declare a content format a "win" with fewer than 5 data points | Anti-hype â€” same discipline as V8 |
| Attribute downloads to a specific post without V8 correlation | Attribution requires cross-dataset join, not guessing |
| Compute engagement-rate trends from 1â€“2 posts | One data point is not a pattern |
...
### 3a â€” Content metadata (recorded at publication time)
...
| `hook` | string | Yes | First 3â€“5 seconds / opening line verbatim or summarized |
...
### 3b â€” Performance metrics (recorded 24â€“72h after posting)

| Field | Type | Required | Notes |
|---|---|---|---|
| `views` | integer | Yes | Total views / plays at time of check |
| `likes` | integer | Yes | Total likes at time of check |
| `comments` | integer | No | Nullable â€” not all platforms show comments easily |
| `saves` | integer | No | Nullable â€” TikTok, Instagram; not available on X/Twitter |
| `shares` | integer | No | Nullable â€” platform-dependent |
| `profile_visits` | integer | No | Nullable â€” TikTok shows this; others may not |
| `website_clicks` | integer | No | Nullable â€” only if link-in-bio analytics available |
| `downloads_attributed` | integer | No | Nullable â€” leave null unless founder manually traced |
| `check_hours_after_post` | integer | No | Hours between posting and metric check (default 48) |

### 3c â€” Approved platforms
...
Unknown platforms: record with `--notes "platform: <name>"` â€” script warns but does not block.

### 3d â€” Content types

```
demo           â€” product demo showing dictation in action
tutorial       â€” how to use a feature step by step
pain_point     â€” problem the founder / user experiences (hook-first)
testimonial    â€” real user story or result
hook_test      â€” testing a specific hook style (trend sound, question, shock)
before_after   â€” typing vs dictation comparison
day_in_life    â€” founder or user routine showing product in context
reaction       â€” reacting to typing struggle / productivity content
```
...
```
manual_founder     â€” real content observation
manual_validation  â€” script testing only (excluded from all reports)
```

### 3g â€” Lesson learned and next action (required free-text)

| Field | Type | Required | Notes |
|---|---|---|---|
| `lesson_learned` | string | Yes (if â‰¥24h since post) | What this post taught. If nothing: "no lesson yet" |
...
**Pass 1 â€” at publication time** (metadata only, no performance metrics yet):
```bash
...
**Pass 2 â€” 24â€“72h after posting** (update with performance metrics):
```bash
python internal/brain/scripts/update_content_observation.py \
    ...
    --lesson_learned "saves higher than likes ratio â€” audience bookmarking for later" \
...
## 5. Future Automated Format (Phase 4 â€” not yet)
...
| `data/benchmark_observations.jsonl` | V7 product metrics (latency, RAM) | V9 Phase 3 â€” when correlating product quality to content reach |
...
### 8a â€” `data/content_observations.jsonl`
...
- `period`: ISO week format `YYYY-Www` â€” must match V8 period keys exactly
...
### 8b â€” Field validation rules

| Field | Validation |
|---|---|
| `platform` | Must be in approved list (Section 3c) â€” warn if not, do not block |
| `content_type` | Must be in approved list (Section 3d) â€” warn if not, do not block |
| `niche` | Must be in approved list (Section 3e) â€” warn if not, do not block |
| `post_date` | Must be valid ISO date `YYYY-MM-DD` |
| `period` | Must match `^\d{4}-W(0[1-9]\|[1-4]\d\|5[0-3])$` |
| `views` | Must be integer â‰¥ 0 |
| `likes` | Must be integer â‰¥ 0 |
| `saves`, `shares`, `comments` | Integer â‰¥ 0 or absent |
| `profile_visits`, `website_clicks` | Integer â‰¥ 0 or absent |
| `downloads_attributed` | Integer â‰¥ 0 or absent â€” must not be inferred, only founder-confirmed |
...
|---|---|---|
| G1 â€” Minimum posts per format | Do not declare a content type a "winner" with fewer than 5 posts | Report shows count; no ranking until â‰¥5 |
| G2 â€” Minimum posts per platform | Do not recommend abandoning a platform with fewer than 5 posts | Report shows count; no abandonment suggestion until â‰¥5 |
| G3 â€” Performance check timing | Do not record performance metrics before 24h post age | Script warns if `check_hours_after_post < 24` |
| G4 â€” V8 join requires real V8 data | Phase 2 correlation requires â‰¥4 weeks of real V8 observations | Script checks V8 baseline before running correlation |
| G5 â€” No attribution without evidence | `downloads_attributed` must be founder-confirmed, not inferred from website_clicks alone | Field is nullable; script never auto-populates it |
| G6 â€” No trend from 1 week | Weekly content snapshot explicitly states "N weeks of data â€” not a trend yet" until â‰¥4 | Anti-hype section in every snapshot |
| G7 â€” No paid recommendation | Report never suggests paid amplification | Hardcoded exclusion from recommendations section |
| G8 â€” Validation exclusion | `source: manual_validation` records are always filtered before any analysis | Same mechanism as V8 |
...
| # | Condition | Action |
|---|---|---|
| SC1 | Founder records views > 500,000 for an early-stage post | Flag for verification â€” potential input error or viral outlier |
| SC2 | Platform analytics show website_clicks >> V8 website_visitors for same week | Flag discrepancy â€” attribution may be measuring different audiences |
| SC3 | Content observations exist but V8 observations do not for the same period | Warn: "V9 distribution data exists for this week but V8 business data is missing â€” Phase 2 correlation will be incomplete" |
| SC4 | Founder records `downloads_attributed` that exceed V8 `downloads` for same period | Flag impossible attribution â€” cannot attribute more downloads than were recorded |
| SC5 | â‰¥3 consecutive weeks with 0 content posts | Flag: "No content recorded for 3+ weeks â€” distribution effort may have paused" |
| SC6 | All recorded posts are from a single platform | Warn: "All data is from one platform â€” cannot assess cross-platform performance" |
| SC7 | Script asked to auto-post, call API, or scrape | Stop immediately â€” outside V9 Phase 1â€“3 scope |
...
- Only produce the join table when â‰¥10 V9 observations and â‰¥4 real V8 weeks both exist
...
V7 found that `paste_execute` takes 645ms â€” 62% of total dictation latency. This is the
visible lag a user sees when the transcription pastes into their active window.

| V7 finding | V9 implication |
|---|---|
| paste_execute = 645ms constant bottleneck | Content demos show a visible paste delay â€” founder should know this is real lag, not editing |
| Idle background inference loop (+110MB/15min) | Long recording demos risk showing RAM warnings on lower-spec machines |
| p95 tail latency = 2405ms (chunk_cleanup) | Demo recordings sometimes show slow outlier â€” not representative of p50 |
| stt_inference_time = 230ms (not the bottleneck) | The inference model is fast â€” demos can honestly highlight inference speed |

**V9 Phase 3 productâ€“content link:**
...
|---|---|---|
| 1 | `add_content_observation.py` â€” CLI recorder for publication + performance | `feat(brain)` |
| 2 | `data/content_observations.jsonl` â€” empty JSONL store | `feat(brain)` |
| 3 | Validate with 2 manual_validation records (publication + performance update) | `feat(brain)` |
| 4 | `review_content_performance.py` â€” report generator | `feat(brain)` |
| 5 | `outputs/content_report.md` â€” generated on validation run | `feat(brain)` |
| 6 | `weekly_content_snapshot.py` â€” founder checklist | `feat(brain)` |
| 7 | `outputs/weekly_content_snapshot.md` â€” generated on validation run | `feat(brain)` |

### Phase 2 â€” V8 correlation (after â‰¥10 V9 observations and â‰¥4 real V8 weeks)

| Step | Deliverable | Type |
|---|---|---|
| 8 | `correlate_content_business.py` â€” joins V9 Ã— V8 by period | `feat(brain)` |
| 9 | `outputs/content_business_correlation.md` â€” weekly join table + commentary | `feat(brain)` |

### Phase 3 â€” Experiment comparison (after â‰¥4 weeks joint data)

| Step | Deliverable | Type |
|---|---|---|
| 10 | `compare_content_experiments.py` â€” before/after format/hook/niche comparisons | `feat(brain)` |
| 11 | `outputs/experiment_report.md` â€” which experiments moved the needle | `feat(brain)` |
| 12 | `outputs/distribution_backlog.md` â€” ranked experiment candidates | updated by above |

### Phase 4 â€” Automation of proven channels (separate design, future)
...
```bash
# Syntax check
python -m py_compile internal/brain/scripts/add_content_observation.py
python -m py_compile internal/brain/scripts/review_content_performance.py
python -m py_compile internal/brain/scripts/weekly_content_snapshot.py
...
python internal/brain/scripts/add_content_observation.py \
    ...
    --saves 5 \
    --check_hours_after_post 48 \
    --lesson_learned "validation sample â€” no lesson" \
    --next_action "validation sample â€” no action" \
    --source manual_validation \
    --period 2026-W18
...
python internal/brain/scripts/weekly_content_snapshot.py

# Confirm validation samples are excluded from reports
grep -c "manual_validation" internal/brain/outputs/content_report.md || echo "excluded correctly"
```
...
V10 will be the first phase where the Brain can make **cross-layer decisions** â€” connecting
...
> distribution performance (V9) â€” what is the single highest-leverage action this week?"

V10 cannot answer this question until all three layers have â‰¥4 weeks of real data.

**V10 gates (preliminary â€” V10 will define its own):**
- V7 Phase 2 baseline locked (benchmark comparison running)
- V8 â‰¥4 weeks real business observations
- V9 â‰¥4 weeks real content observations across â‰¥2 platforms
...
Mission:
Implement V9 Phase 1 â€” Content Performance Recorder.
...
Create:
- internal/brain/scripts/add_content_observation.py
- internal/brain/data/content_observations.jsonl  (empty JSONL store)
- internal/brain/scripts/review_content_performance.py
- internal/brain/scripts/weekly_content_snapshot.py
- internal/brain/outputs/content_report.md         (generated)
- internal/brain/outputs/weekly_content_snapshot.md (generated)
...
- Section 9: safety gates (â‰¥5 posts before ranking, â‰¥24h before performance check)
...
- Sections: platform breakdown, content_type breakdown, top posts by views,
  hook pattern summary (tabular â€” no NLP), V8 connection placeholder,
...
- Shows: posts this week, best performer (if â‰¥1), pending performance checks,
  lesson summary (list of lesson_learned fields), next actions (list of next_action fields)
- Anti-hype section: "N weeks of data â€” not a trend yet" until â‰¥4
...
- Follow brain.py (from brain module) for file I/O â€” use append_jsonl, write_text, read_jsonl.
```
...
Period key:  ISO week YYYY-Www (same as V8 â€” enables cross-dataset join)

Scripts to build (Phase 1):
  add_content_observation.py   â€” CLI recorder (publication + performance update)
  review_content_performance.py â€” content report + distribution backlog
  weekly_content_snapshot.py   â€” founder checklist (anti-hype)

Safety:
  - No ranking until â‰¥5 posts of a type (gate G1/G2)
  - No trend until â‰¥4 weeks (gate G6)
...
V8 link:    join by period â€” content weeks Ã— business weeks
V7 link:    paste latency known â€” demos show real p50 lag
V10 link:   all three layers (V7+V8+V9) join by period â†’ unified decision engine

Phases:
  Phase 1 â€” Manual content log         â† NEXT (implement now)
  Phase 2 â€” V8 correlation             (after â‰¥10 posts + â‰¥4 V8 weeks)
  Phase 3 â€” Experiment comparison      (after â‰¥4 joint weeks)
  Phase 4 â€” Channel automation         (separate design â€” not now)
