"""generate_product_patch_proposal.py — V5 Product Patch Proposal Mode.
...
def _write_report(
    ...
    lines.extend([
        "",
        f"Sensitive files involved: {'YES — ' + ', '.join(sensitive_files) if sensitive else 'No'}",
        f"Risk: {risk}",
        "",
        "## Proposed Changes",
        "",
...
def main() -> None:
    ...
    if best is None:
        ...
        print("Written vocalype-brain/outputs/product_patch_proposal_report.md")
        return
    ...
    validation_plan = (
        validation_test
        or "1. Manual test: activation_failed state — confirm message is clear and retry button works.\n"
           "2. Manual test: checking_activation — confirm spinner does not hang indefinitely.\n"
           "3. Run: npm run lint\n"
