"""review_product_patch_proposal.py â€” V5 Product Patch Proposal Review.
...
def _is_safe_to_request(proposal: dict[str, Any]) -> tuple[bool, str]:
    """Return (safe, reason) â€” whether it is safe to send this to an implementation model."""
    if proposal.get("status") == "refused":
        return False, "Proposal was refused â€” no safe candidate available."

    risk = proposal.get("risk", "high")
    if risk == "high":
        return False, "Risk is HIGH â€” do not send to implementation model without founder review."

    if proposal.get("sensitive_files_involved"):
        return False, "Sensitive files are involved â€” requires explicit founder approval before implementation."

    target_files = proposal.get("target_files", [])
    if not target_files:
        return False, "No target files defined â€” proposal is too vague to implement safely."
...
def main() -> None:
    ...
    divider = "=" * 60
    print(divider)
    print("Vocalype Brain â€” Product Patch Proposal Review")
    print(divider)

    if not proposals:
        print("\nNo product patch proposals found.")
        print("Run: python internal/brain/scripts/generate_product_patch_proposal.py")
        print(divider)
        return
    ...
    print(f"\nSafe to request   : {'YES â€” ' + safety_reason if safe else 'NO â€” ' + safety_reason}")
    ...
    if len(proposals) > 1:
        ...
        for p in proposals[-6:]:
            print(f"  [{p.get('date','?')}] {p.get('risk','?')} / {p.get('status','?')} â€” {p.get('title','?')}")
        print(divider)
