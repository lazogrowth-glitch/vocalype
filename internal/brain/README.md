...
It turns product observations, benchmark data, business metrics, and content performance into a weekly prioritised action â€” then generates a safe, scoped mission package ready to send to Claude, Codex, or Aider.

---

## âš¡ Mode OpÃ©rationnel â€” Construction V1â€“V12 terminÃ©e

**Le cerveau est construit. Il fonctionne maintenant en Mode OpÃ©rationnel.**

Utilise les lanceurs sur ton Bureau Windows. Tu n'as pas besoin de taper des commandes.

### Bureau Windows â€” UN seul bouton principal
...
Les autres boutons sont rangÃ©s dans le dossier **`Vocalype Brain - AvancÃ©`** sur le Bureau.
Tu n'en as besoin que dans des cas spÃ©cifiques (voir ci-dessous).

### Workflow normal (4 Ã©tapes)

```
1. Double-clic : Lancer Agent Vocalype Auto
   -> rapport gÃ©nÃ©rÃ© -> action classifiÃ©e -> recommendation ouverte

2. Lis : agent_recommendation.md
   -> que faire cette semaine ?

3. Si mission produit nÃ©cessaire :
   -> copie v11_mission_package.md dans Claude / Codex
   -> laisse Claude exÃ©cuter
   -> review le diff avant de commit

4. AprÃ¨s que Claude ait fini (si besoin) :
   -> dossier AvancÃ© -> Enregistrer Resultat.bat
```

**Pas de V13. Le Mode OpÃ©rationnel continue.**

### Dossier AvancÃ© â€” Bureau / Vocalype Brain - AvancÃ©

Ces boutons existent mais ne sont pas nÃ©cessaires au quotidien :

| Fichier | Quand l'utiliser |
|---|---|
| `Lancer Vocalype Brain.bat` | Cycle manuel sans l'agent auto |
| `Voir Action du Robot.bat` | Ouvrir `weekly_action.md` directement |
| `Generer Mission Claude.bat` | RÃ©gÃ©nÃ©rer le package mission manuellement |
| `Enregistrer Resultat.bat` | AprÃ¨s qu'une mission Claude soit terminÃ©e |
| `Voir Rapports Vocalype Brain.bat` | Parcourir tous les rapports disponibles |
| `Stop Vocalype Brain.bat` | Rappel : pas de daemon, rien Ã  stopper |
| `Creer Context DeepSeek.bat` | PrÃ©parer `context_pack.md` pour DeepSeek / Claude manuellement |

### Agent Auto â€” Routage intelligent

`Lancer Agent Vocalype Auto` dÃ©cide automatiquement du bon outil :

- **TÃ¢che routine / rapport** â†’ outils locaux uniquement, rien ne quitte la machine
- **Raisonnement long-contexte** â†’ DeepSeek (si configurÃ© et autorisÃ©) ou instructions pour Claude manuel
- **ImplÃ©mentation produit sensible** â†’ package mission Claude/Codex, jamais automatique

**Mode externe** (`VOCALYPE_BRAIN_EXTERNAL_MODE`) :
- `off` â†’ jamais d'API externe, prÃ©pare `context_pack.md` seulement
- `confirm` â†’ **(dÃ©faut)** prÃ©pare `context_pack.md` + instructions fondateur, n'appelle PAS l'API
- `auto` â†’ appelle DeepSeek pour les tÃ¢ches `long_reasoning` si `DEEPSEEK_API_KEY` est configurÃ©

Le code produit n'est **jamais** modifiÃ© automatiquement par ce routeur.
...
C'est un **systÃ¨me d'exploitation pour une boucle** :

```
mesurer â†’ dÃ©cider â†’ mission â†’ exÃ©cuter â†’ analyser â†’ apprendre
```

Chaque script se lance, fait son travail, et s'arrÃªte. Tu restes aux commandes.

### Routage des modÃ¨les

| ModÃ¨le | TÃ¢ches | DonnÃ©es envoyÃ©es |
|---|---|---|
| **Ollama local** (qwen3:8b, qwen2.5-coder:7b) | Rapports, rÃ©sumÃ©s, prioritÃ©s, gÃ©nÃ©ration missions | Rien ne quitte ta machine |
| **DeepSeek Flash** (si `DEEPSEEK_API_KEY` configurÃ©) | Raisonnement long-contexte, analyse multi-fichiers | Seulement `context_pack.md` (mÃ©moire Brain uniquement) |
| **Claude / Codex / Aider** | ImplÃ©mentation produit sensible â€” Rust, auth, paiement | Seulement `v11_mission_package.md` â€” toujours copiÃ©/collÃ© manuellement |

Le cerveau gÃ©nÃ¨re le package mission. Toi, tu choisis quel modÃ¨le l'exÃ©cute.

**ClÃ©s API :** jamais commises dans git. Configure `DEEPSEEK_API_KEY` comme variable d'environnement Windows ou dans un `.env` local exclu du dÃ©pÃ´t. VÃ©rifie avec `Creer Context DeepSeek.bat`.

```bash
# VÃ©rifier la configuration DeepSeek
python internal/brain/scripts/check_deepseek_setup.py

# Construire le pack de contexte pour modÃ¨le externe
...
```bash
python internal/brain/scripts/daily.py
```

This scores `internal/brain/data/actions.jsonl`, selects the top 5 actions, and writes:

```txt
internal/brain/outputs/daily_actions.md
```
...
```bash
python internal/brain/scripts/add_feedback.py "User says the license activation is confusing"
```

This appends a classified item to:

```txt
internal/brain/data/feedback.jsonl
```
...
```bash
python internal/brain/scripts/add_experiment.py
```
...
```txt
internal/brain/data/experiments.jsonl
```

Review experiments with:

```bash
python internal/brain/scripts/review_experiments.py
```

This writes:

```txt
internal/brain/outputs/weekly_review.md
```

## Generate Content Ideas

```bash
python internal/brain/scripts/generate_content_ideas.py
```
...
```txt
internal/brain/outputs/growth_report.md
```
...
```bash
python internal/brain/scripts/model_benchmark_template.py
```

This creates:

```txt
internal/brain/outputs/model_report.md
internal/brain/data/benchmarks.jsonl
```
...
```bash
python internal/brain/scripts/score_actions.py
```

Scoring uses:

```txt
internal/brain/config/scoring.config.json
```
...
```txt
internal/brain/config/brain.config.json
```
...
```bash
python internal/brain/scripts/model_router.py
```

Run from repo root:

```bash
python internal/brain/scripts/orchestrator.py daily
python internal/brain/scripts/orchestrator.py ask "What should we improve today in Vocalype?"
python internal/brain/scripts/orchestrator.py growth
python internal/brain/scripts/orchestrator.py product
python internal/brain/scripts/orchestrator.py self-improve
```
...
```bash
python internal/brain/scripts/index_memory.py
python internal/brain/scripts/retrieve_context.py "license activation"
python internal/brain/scripts/orchestrator.py ask "What should I improve today?"
```
...
- `internal/brain/data/night_shift_runs.jsonl`
- `internal/brain/data/proposed_patches.jsonl`
- `internal/brain/outputs/night_shift_report.md`
...
```bash
python internal/brain/scripts/add_quality_observation.py "Dictation feels slow on first run"
...
- `internal/brain/memory/quality_playbook.md`
- `internal/brain/data/quality_observations.jsonl`
- `internal/brain/data/performance_metrics.jsonl`
- `internal/brain/outputs/quality_report.md`
...
```bash
python internal/brain/scripts/record_result.py
```

Files used:

- `internal/brain/data/results.jsonl`
- `internal/brain/outputs/implementation_review.md`
- `internal/brain/outputs/results_report.md`
- `internal/brain/memory/lessons_learned.md`
- `internal/brain/memory/wins.md`
- `internal/brain/memory/mistakes.md`
...
- writes only inside `internal/brain`
...
- `internal/brain/outputs/codex_task.md`
- `internal/brain/data/approved_task_candidates.jsonl`
...
- `planning_only` â€” vague, high-risk, or no concrete behavior change. Generates a clarification prompt only. No product code modifications.
- `measurement_task` â€” proposal intent is to measure, track, observe, or map failure points. Generates a measurement plan prompt. No product code modifications yet.
- `implementation_task` â€” concrete code or UI behavior change with clear approved scope, validation test, and low/medium risk.
...
- `internal/brain/patches/patch_YYYYMMDD_HHMMSS_<slug>.md` â€” patch proposal file
- `internal/brain/outputs/safe_patch_report.md` â€” latest patch summary
- `internal/brain/data/safe_patch_candidates.jsonl` â€” full patch history

Safety classes:

| Class | Meaning | Auto-apply? |
|---|---|---|
| `brain_safe` | Only `internal/brain/` files targeted | No |
...
- if `brain_safe` or `docs_safe`: generates a patch proposal file in `internal/brain/patches/`
- if `product_proposal_only`: generates a text-only proposal â€” no product file is written
...
```bash
# Dry run (default â€” no files modified)
python internal/brain/scripts/apply_approved_patch.py

# Apply (requires explicit flag)
python internal/brain/scripts/apply_approved_patch.py --approve
```

Outputs:

- `internal/brain/outputs/apply_patch_report.md` â€” apply result
- `internal/brain/data/applied_patches.jsonl` â€” application history
...
target_file: internal/brain/memory/some_file.md
operation: append
content:
- New line to append
- Another line
...
- anything inside `internal/brain/`
- `README` files
- `docs/` files
...
- dry-run by default â€” `--approve` required for any write
...
```bash
python internal/brain/scripts/generate_product_patch_proposal.py
python internal/brain/scripts/review_product_patch_proposal.py
```

Outputs:

- `internal/brain/outputs/product_patch_proposal_report.md` â€” full proposal with implementation prompt
- `internal/brain/data/product_patch_proposals.jsonl` â€” proposal history
...
- It cannot write outside `internal/brain`.
...
```txt
internal/brain/scripts/tools.py
```
...
```bash
python internal/brain/scripts/self_improvement.py
```

This writes:

```txt
internal/brain/data/self_improvements.jsonl
internal/brain/outputs/improvement_proposals.md
```
