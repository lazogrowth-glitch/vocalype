...
def _worktree_warning(candidate: dict[str, Any]) -> str:
    ...
    if not changed:
        return ""
    product_changes = [
        path for path in changed if not path.startswith("internal/brain/") and not path.endswith(".bat")
    ]
...
def _validation_commands(candidate: dict[str, Any]) -> list[str]:
    ...
    if candidate.get("source") == "quality":
        commands.append("python internal/brain/scripts/review_quality.py")
...
def _measurement_prompt(candidate: dict[str, Any], critic_review: str, worktree_warning: str, scope_reason: str) -> str:
    ...
    output_file = f"internal/brain/outputs/{slug}.md"
    ...
    lines.extend([
        ...
        f"- write the measurement plan inside internal/brain/outputs/ as {output_file}",
        "",
        "Forbidden:",
...
def _deterministic_prompt(candidate: dict[str, Any], critic_review: str, worktree_warning: str, scope_reason: str) -> str:
    ...
    if mode == "planning":
        lines = [
            "# Mission Codex â€” Clarify Next Safe Vocalype Task",
            "",
    ...
    lines = [
        "# Mission Codex â€” Implement Approved Vocalype Task",
        "",
        f"Task title: {candidate.get('title', 'Untitled')}",
        "",
        "Original proposal summary:",
...
def main() -> None:
    ...
    record = {
        ...
        "generated_prompt_path": "internal/brain/outputs/codex_task.md",
    }
    append_jsonl("data/approved_task_candidates.jsonl", record)

    print("Generated internal/brain/outputs/codex_task.md")
