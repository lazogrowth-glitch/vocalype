...
Patch is `brain_safe`. Changes are limited to vocalype-brain/ memory files.
...
After applying:
1. Read vocalype-brain/memory/lessons_learned.md — confirm the new line is present.
2. Confirm no product code was modified (git status --short should show only vocalype-brain/ files).
...
- Revert with `git checkout -- vocalype-brain/memory/lessons_learned.md` if needed.

## Rollback Plan

```
git checkout -- vocalype-brain/memory/lessons_learned.md
```
