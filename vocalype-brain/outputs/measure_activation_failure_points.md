# Activation Failure Points — Measurement Plan

Date: 2026-04-24
Source: Vocalype Brain V2 measurement task
Status: Plan only — no product code modified
...
| 2 | `logged_out` | No token found → `applySession(null)` |
| 3 | User opens login via browser | Deep link or form flow starts |
| 4 | Auth callback received | Token stored via deep link → `handleDeepLinkAuth()` |
...
| 7a | License issued/refreshed OK | `licenseState.state = "online_valid"` or `"offline_valid"` → `ready` |
| 7b | Subscription inactive | `session.subscription.has_access = false` → `subscription_inactive` |
| 7c | License sync fails (403) | `licenseState` cleared → `activation_failed` |
...
Source files:
- `src/hooks/useAuthFlow.ts` — `deriveActivationStatus`, `refreshSession`, `syncLicenseForSession`
- `src/components/auth/AuthPortal.tsx` — `getStatusText`, `getReadinessRows`
...
### F1 — `activation_failed` state with no actionable guidance
...
### F2 — `checking_activation` with no timeout or progress indicator
...
### F3 — `subscription_inactive` → billing CTA goes to external browser
...
### F4 — Silent suppression of `no stored license bundle` error
- **What happens:** `isExpectedMissingLicenseMessage` returns `true` → error is suppressed → user sees no feedback.
- **Problem:** Silent failure — user doesn't know activation is stalled on missing bundle.
- **Code location:** `useAuthFlow.ts:18-26`, `useAuthFlow.ts:180-185`, `useAuthFlow.ts:303-309`

### F5 — Deep link auth failure (browser callback never arrives)
...
### F6 — Token expired at launch → silent refresh attempt → falls through to logout
- **What happens:** Access token is 401 → tries refresh token → if that fails → tries offline license → if that fails → clears everything.
...
### F7 — `activation_failed` due to integrity anomaly
- **What happens:** `runtime.integrity_anomalies` detected → anomaly reported silently → but user still sees `activation_failed` UI.
...
(Once a tracking plan is approved by founder — no implementation yet.)
...
All events would be append-only to a local Brain log first — no third-party analytics without founder approval.
...
- [ ] Time the full flow: install → login → `ready` → first dictation (target < 60s)
...
**Recommended starting point:** Option A + B — both are frontend-only, low risk, and directly address the most common failure surfaces (F1, F2).
...
| Analytics could leak data | Any event tracking must be local-only first | Brain JSONL only — no third-party API without approval |
...
Manual observation is sufficient to validate whether F1–F7 are actually causing user drop-off, or whether they are edge cases. Recommended next steps:

1. Do the manual observation checklist in section 6 on a real build.
2. Record observations in `vocalype-brain/data/quality_observations.jsonl`.
