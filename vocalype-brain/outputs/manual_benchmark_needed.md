# Vocalype V7 — Manual Benchmark Collection Guide
...
| Model on disk | `parakeet-tdt-0.6b-v3-int8` — **639.6 MB** |
| App data dir | `%APPDATA%\com.vocalype.desktop\` |
| Typical dictation length | 1.4s – 5.5s (from 10 recent recordings) |
...
- `stop_recording` — audio capture stop time
- `chunk_finalize_and_assemble` — chunking assembly time
- `parakeet_full_audio_recovery` — recovery path time (when used)
- `total_duration_ms` — full pipeline duration (trigger → paste)
...
**Status: MANUAL REQUIRED** — App was not running at collection time.
...
4. Open **Task Manager** → Details tab → find `vocalype.exe`.
...
**Expected range:** 150–600 MB (model loaded in RAM).
Model file is 639.6 MB on disk — expect RAM footprint to be similar or higher.

---

## Metric 2: `ram_during_transcription_mb`

**Status: MANUAL REQUIRED** — No safe way to trigger transcription automatically.

**How to measure:**
1. Launch Vocalype. Wait for "ready" state.
2. Open **Task Manager** → Details tab → find `vocalype.exe`.
3. Trigger a 5–10 second dictation (speak clearly).
4. **While the spinner is showing** (transcribing state), note the peak RAM.
5. Record the highest value seen during transcription.

> Tip: Task Manager updates every ~1 second. You may need 2–3 tries to catch the peak.
...
**Status: MANUAL REQUIRED** — Requires active dictation to have just completed.

**How to measure:**
1. After a dictation completes (text pasted), wait **5 seconds**.
2. Record the `vocalype.exe` Working Set from Task Manager.
3. Compare to `app_idle_ram_mb` — the delta reveals memory leaks.
...
**Status: MANUAL REQUIRED** — No existing log captures this on disk.
...
**Status: MANUAL REQUIRED** — But readable from dev console automatically (no instrumentation).

### Option A — Dev mode terminal (recommended, precise)

1. Run `bun run tauri dev` in a terminal from the project root.
2. Trigger a normal dictation (hotkey → speak → release).
...
4. The `total=Xms` value IS `total_dictation_latency_ms` (pipeline start → paste complete).

### Option B — Stopwatch (manual, installed app)
...
**Status: MANUAL REQUIRED** — But readable from dev console automatically.

### Dev mode terminal (recommended)

Same as Metric 5 — run `bun run tauri dev` and read the pipeline profile log.
...
## Recommended Session Protocol (M1 — First Manual Session)

Run all of the following in one 30-minute session:

| Step | Action | Metrics collected |
|---|---|---|
| 1 | Open terminal. Run `bun run tauri dev`. | — |
| 2 | Wait for app to reach "ready" state. | `model_load_time_ms` (stopwatch) |
| 3 | Open Task Manager → vocalype.exe → record Working Set. | `app_idle_ram_mb` |
...
| 8 | Run `python vocalype-brain/scripts/review_benchmarks.py`. | Updated report |

**Target after one session:** 6 metrics × 1 observation = 6 total. Need 4 more sessions to reach baseline.

---

## Context: Existing Benchmark Observations

| Metric | Observations so far | Baseline ready (≥5) |
|---|---|---|
| `total_dictation_latency_ms` | 1 (validation sample: 2400ms) | ❌ |
| All others | 0 | ❌ |
...
## After Collecting ≥5 Observations Per Metric
...
When all 10 priority metrics have ≥5 observations, the report will recommend
locking the baseline with `lock_benchmark_baseline.py --approve` (V7 Phase 2 — not yet built).

---

*This file is generated by Vocalype Brain — V7 Phase 1. Do not modify product code to collect these metrics.*
