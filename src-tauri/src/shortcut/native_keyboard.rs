//! Native keyboard backend implementation
//!
...
/// State for the native keyboard shortcut manager
...
impl NativeKeyboardState {
    /// Create a new NativeKeyboardState
    ...
    fn manager_thread(cmd_rx: Receiver<ManagerCommand>, app: AppHandle) {
        info!("Native keyboard manager thread started");
        ...
        loop {
            ...
            while let Some(event) = manager.try_recv() {
                if let Some((binding_id, hotkey_string)) = hotkey_to_binding.get(&event.id) {
                    debug!(
                        "Native keyboard event: binding={}, hotkey={}, state={:?}",
            ...
            match cmd_rx.recv_timeout(std::time::Duration::from_millis(10)) {
                Ok(cmd) => match cmd {
                    ...
                    ManagerCommand::Shutdown => {
                        info!("Native keyboard manager thread shutting down");
                        break;
                    }
                },
        ...
        info!("Native keyboard manager thread stopped");
    }
    ...
    fn do_register(
        ...
        debug!(
            "Registered native keyboard shortcut: {} -> {:?}",
            binding_id, hotkey
        );
        Ok(())
    }
    ...
    pub fn start_recording(&self, app: &AppHandle, binding_id: String) -> Result<(), String> {
        ...
        debug!("Started native keyboard recording mode");
        Ok(())
    }
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<NativeKeyboardState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                let frontend_event = FrontendKeyEvent {
                    ...
                    hotkey_string: key_event
                        .as_hotkey()
                        .map(|h| h.to_portable_string())
                        .unwrap_or_default(),
                };

                // Emit to frontend
                if let Err(e) = app.emit("native-keyboard-event", &frontend_event) {
    ...
    pub fn stop_recording(&self) -> Result<(), String> {
        ...
        debug!("Stopped native keyboard recording mode");
        Ok(())
    }
}

impl Drop for NativeKeyboardState {
    fn drop(&mut self) {
...
/// Validate a shortcut string for the native keyboard implementation.
/// The native keyboard backend is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // The native keyboard backend accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for native keyboard backend: {}", e))
}

/// Initialize native keyboard shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = NativeKeyboardState::new(app.clone())?;
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            ...
            error!(
                "Failed to register native keyboard shortcut {} during init: {}",
                id, e
            );
        }
    }
    ...
    app.manage(state);
    info!("Native keyboard shortcuts initialized");
    Ok(())
}
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<NativeKeyboardState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<NativeKeyboardState>()
        .ok_or("NativeKeyboardState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn stop_native_keyboard_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::NativeKeyboard {
        return Err("Native keyboard backend is not the active keyboard implementation".into());
    }
