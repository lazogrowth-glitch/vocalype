...
def _is_product_file(path: str) -> bool:
    ...
    ) and not path.startswith("internal/brain/")
...
def _recommended_action(safety_class: str, status: str) -> list[str]:
    ...
    if safety_class == "product_proposal_only":
        return [
            "This is a PROPOSAL ONLY â€” do not apply automatically.",
    ...
    return ["Unknown safety class â€” review manually before proceeding."]
...
def main() -> None:
    ...
    divider = "=" * 60
    print(divider)
    print("Vocalype Brain â€” Safe Patch Review")
    print(divider)
    ...
    print(f"Product code involved  : {'YES â€” ' + ', '.join(product_files) if product_files else 'No'}")
    ...
    print(f"\nTotal patch files in internal/brain/patches/ : {total}")
    if patch_files:
