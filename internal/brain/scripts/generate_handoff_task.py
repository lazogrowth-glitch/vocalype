"""generate_handoff_task.py â€” V6 Product Implementation Handoff Loop.
...
Dry-run by default â€” pass --approve to write output files.
"""
...
# Project root (parent of internal/brain)
...
def _is_vague(proposed_changes: list[Any]) -> bool:
    ...
    for change in proposed_changes:
        ...
        # or a concrete UI element â€” not just a generic verb with no noun.
...
def _run_safety_gates(
    ...
    # G1 â€” proposal must exist (caller already checked non-empty list, but defensive)
    if not proposal:
        return "STOP: no product proposals found â€” run generate_product_patch_proposal.py first"

    # G2 â€” status must be "proposed"
    status = proposal.get("status", "")
    if status != "proposed":
        return f'STOP: proposal status is "{status}" â€” only "proposed" proposals can be handed off'

    # G3 â€” manual_approval_required must be true
    if not proposal.get("manual_approval_required", False):
        return "STOP: proposal missing manual approval flag â€” do not hand off without founder confirmation"

    # G4 â€” risk must not be high
    risk = str(proposal.get("risk", "high")).lower()
    if risk == "high":
        return "STOP: proposal risk is HIGH â€” founder must narrow scope before handoff"

    # G5 â€” no sensitive files
    if proposal.get("sensitive_files_involved", True):
        return "STOP: sensitive files detected in proposal â€” requires explicit founder approval"

    # G6 â€” target files must be defined
    target_files = proposal.get("target_files", [])
    if not target_files:
        return "STOP: no target files in proposal â€” return to measure â†’ propose cycle"

    # G7 â€” forbidden patterns (string check)
    for f in target_files:
        if _is_forbidden(str(f)):
            return f"STOP: forbidden file in scope: {f} â€” remove from target_files and re-propose"

    # G7b â€” forbidden patterns (absolute path check)
    for f in target_files:
    ...
    # G8 â€” deduplication against applied_patches (applied status only)
    applied_titles = {
    ...
    for applied_title in applied_titles:
        if applied_title and applied_title in proposal_title:
            ...
            return f"STOP: this patch was already applied ({applied_date}) â€” verify current state before re-applying"

    # G10 â€” brain config safety key
    safety = config.get("safety", {})
    if "allow_product_code_modifications" not in safety:
        return "STOP: brain config missing safety key â€” do not proceed"

    # G11 â€” safety key must be false
    if safety.get("allow_product_code_modifications") is True:
        return "STOP: safety config anomaly â€” allow_product_code_modifications must be false"

    # G9 â€” target files must be readable (checked last â€” file I/O)
    for f in target_files:
        ok, _ = _read_product_file_safe(str(f))
        if not ok:
            return f"STOP: cannot read target file {f} â€” verify path and permissions"
...
def _build_handoff_task_md(
    ...
    if task_type == "planning_only":
        lines.extend([
            ...
            "",
            f"**Stop reason:** {gate_failure or 'Proposed changes too vague â€” return to measure â†’ diagnose â†’ propose cycle.'}",
            "",
    ...
    # --- implementation_task â€” full handoff document ---
    lines.extend([
        "## Problem Statement",
        "",
        summary or title,
    ...
    lines.extend([
        ...
        "Do not run benchmarks now â€” these are placeholders only.",
        "",
        "| Metric | Before | After |",
        "|---|---|---|",
    ])
...
def main(approve: bool = False) -> None:
    ...
    divider = "=" * 60
    print(divider)
    print("Vocalype Brain â€” V6 Generate Handoff Task")
    print(divider)
    ...
    if not proposals:
        stop = "STOP: no product proposals found â€” run generate_product_patch_proposal.py first"
        print(f"\n{stop}")
        if approve:
    ...
    if gate_failure:
        ...
        if approve:
            ...
            print("\nWritten: outputs/handoff_task.md  (planning_only â€” gate failure)")
        else:
            print("\n[DRY-RUN] Would write: outputs/handoff_task.md  (planning_only â€” gate failure)")
        print(divider)
        sys.exit(1)
