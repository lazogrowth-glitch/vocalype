"""review_handoff_task.py — V6 Product Implementation Handoff Review.
...
def _safe_to_send(record: dict[str, Any]) -> tuple[bool, str]:
    """Return (safe, reason) — whether this handoff is safe to send to an implementation model."""
    task_type = record.get("task_type", "")
    status = record.get("status", "")
    stop_reason = record.get("stop_reason", "")

    if status == "refused" or stop_reason:
        return False, stop_reason or "Handoff was refused — see stop_reason."

    if task_type == "planning_only":
        return False, "Task type is planning_only — resolve stop condition first."

    if task_type == "measurement_task":
        return False, "Task type is measurement_task — complete manual observations first."

    if task_type == "proposal_task":
        return False, "Task type is proposal_task — narrow scope and re-run generate_product_patch_proposal.py."
    ...
    if not target_files:
        return False, "No target files — too vague to implement safely."
    ...
    if risk == "high":
        return False, "Risk is HIGH — founder must narrow scope first."
...
def _recommended_action(record: dict[str, Any], safe: bool, reason: str) -> list[str]:
    ...
    if not record:
        return [
            ...
            "Run: python vocalype-brain/scripts/generate_handoff_task.py --approve",
        ]
    ...
    if safe:
        return [
            "Read outputs/handoff_task.md — review the full handoff document.",
...
def main() -> None:
    ...
    divider = "=" * 60
    print(divider)
    print("Vocalype Brain — V6 Handoff Task Review")
    print(divider)

    if not records:
        print("\nNo handoff tasks found.")
        print("Run: python vocalype-brain/scripts/generate_handoff_task.py --approve")
        print(divider)
        return
    ...
    print(f"\nSafe to send       : {'YES — ' + reason if safe else 'NO — ' + reason}")
    ...
    if len(records) > 1:
        ...
        for r in records[-6:]:
            print(f"  [{r.get('date','?')}] {r.get('task_type','?')} / {r.get('status','?')} — {r.get('title','?')}")
        print(divider)
