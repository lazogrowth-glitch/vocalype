"""
build_context_pack.py — Vocalype Brain Operating Mode
======================================================
...
What it does:
  ...
  - Writes vocalype-brain/outputs/context_pack.md
...
Usage:
  ...
  python vocalype-brain/scripts/build_context_pack.py --extra path/to/file.md
"""
...
OPTIONAL_FILES: list[tuple[str, str]] = [
    (
        "vocalype-brain/outputs/v11_mission_package.md",
...
# ---------------------------------------------------------------------------
# Forbidden path patterns — never included regardless of arguments
# ---------------------------------------------------------------------------
...
def build_context_pack(extra_files: list[Path] | None = None) -> Path:
    ...
    for ef in extra_files:
        ...
        if forbidden:
            print(f"[BLOCKED] '{ef}' matches forbidden pattern '{reason}' — not included.")
    ...
    # ---- Header ----
    sections.append(
        f"# Vocalype Brain — Context Pack\n\n"
        f"**Generated:** {timestamp}\n\n"
        f"> **This context pack may be sent to an external model.**\n"
        f"> It contains only Brain memory and output files — no product code,\n"
    ...
    for rel_path, description in DEFAULT_FILES:
        ...
        if forbidden:
            print(f"[SKIP] {rel_path} — forbidden pattern '{reason}'")
            continue
    ...
    for ef in extra_files:
        ...
        forbidden, reason = _is_forbidden(abs_path)
        if forbidden:
            print(f"[SKIP] {ef} — forbidden pattern '{reason}'")
            continue
        content = _read_file(abs_path)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Additional Brain files to include (must be inside vocalype-brain/)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
