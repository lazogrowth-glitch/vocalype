"""add_business_observation.py â€” V8 Phase 1 Manual Business Metrics Recorder.
...
Statuses:
  measured      â€” data source opened, real numeric value recorded
  zero          â€” data source opened, confirmed value is 0
  unknown       â€” data source not checked this week (no value)
  not_available â€” data source does not exist yet (no value)
  not_applicable â€” metric not relevant at current stage (no value)
...
def _resolve_status(args: argparse.Namespace) -> tuple[str, list[str]]:
    ...
    if status == "measured":
        if args.value is None:
            ...
            errors.append(f"--status measured: value {args.value} is negative â€” verify this is intentional.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="V8 Manual Business Metrics Recorder â€” append one observation.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Statuses:
  measured      â€” real numeric value from dashboard
  zero          â€” confirmed zero (dashboard opened, result is 0)
  unknown       â€” not checked this week
  not_available â€” data source does not exist yet
  not_applicableâ€” metric not relevant at current stage
...
Examples:
  ...
  add_business_observation.py --metric churned_users --status not_applicable \\
    ...
    print(divider)
    print("Written: internal/brain/data/business_observations.jsonl")
