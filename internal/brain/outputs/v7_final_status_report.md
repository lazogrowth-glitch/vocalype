# Vocalype Brain â€” V7 Final Status Report
...
|---|---|---|
| `v7_design_plan.md` â€” 13-section benchmark architecture | âœ… Done | Planning only, no scripts implied |
| `add_benchmark_observation.py` â€” CLI observation recorder | âœ… Done | Validated, committed |
| `review_benchmarks.py` â€” report generator | âœ… Done | Validated, generates benchmark_report.md |
| `manual_benchmark_needed.md` â€” M1 session guide | âœ… Done | Machine profile, protocol, exact commands |
| Historical data mining â€” SQLite history.db + WAV files | âœ… Done | 1124 records inspected, WAV durations extracted |
| Persisted log search â€” vocalype.log discovery | âœ… Done | 157 KB, 43 Pipeline profile entries found |
| `pipeline_logs_search_report.md` â€” log extraction report | âœ… Done | Step breakdowns, raw log lines, 38 complete runs |
| `benchmark_report.md` â€” auto-generated baseline readiness | âœ… Done | 37 observations, 2/10 metrics covered |
| `v7_bottleneck_hypothesis.md` â€” 10-section hypothesis report | âœ… Done | H1 paste-bound (medium-high), H2 chunk_cleanup (high) |
| `product_patch_proposal_report.md` â€” investigation proposal | âœ… Done | paste_execute investigation, type: investigation_only |

**What V7 did NOT complete (by design):**

| Deliverable | Status | Reason |
|---|---|---|
| `lock_benchmark_baseline.py` â€” Phase 2 script | âŒ Not built | 8/10 metrics still at 0 obs â€” premature |
| `compare_benchmarks.py` â€” Phase 2 script | âŒ Not built | No baseline to compare against |
| `benchmark_baseline.jsonl` â€” locked baseline | âŒ Not created | Requires â‰¥5 obs for all 10 metrics |
| M1â€“M7 manual sessions â€” RAM, WER, activation | âŒ Not run | Requires app to be running; deferred to next session |
| paste_execute root-cause investigation | âŒ Not run | Per contract: no product code inspection in planning phase |
| V7 Phase 3 â€” automated benchmark runner | âŒ Not designed | Requires Phase 2 first |
...
| `chunk_finalize_and_assemble` | 303 ms | 886 ms | Variable â€” includes inference |
...
| `paste_execute` | **645 ms** | 687 ms | **Remarkably constant â€” Â±35 ms across 38 runs** |
...
| min | 178 ms |
| max | 565 ms |
| Real-time factor | ~0.06x (well under 1.0x â€” faster than real time) |

### Startup Timing (7 launches, from `[startup]` log lines)

| Step | Range | Average |
|---|---|---|
| `initialize_core_logic` | 30â€“44 ms | 35 ms |
| `Microphone stream initialized` | 139â€“239 ms | 197 ms |
...
A baseline is ready when â‰¥5 observations exist for a metric.

| Metric | Observations | Baseline Ready | Notes |
|---|---|---|---|
| `total_dictation_latency_ms` | **9** | âœ… **Yes** | p50=1043ms, p95=2405ms, min=717ms |
| `stt_inference_time_ms` | **20** | âœ… **Yes** | p50=230ms, p95=459ms, min=178ms |
| `paste_latency_ms` (extra, not priority) | 5 | âœ… Yes (extra) | mean=652ms, range 618â€“687ms |
| `capture_duration_ms` (extra, partial) | 3 | âŒ No | Only 3 samples â€” not priority metric |
...
|---|---|---|---|
| `model_load_time_ms` | 0 | 5 needed | Cold start stopwatch â€” app launch â†’ "ready" |
| `app_idle_ram_mb` | 0 | 5 needed | Task Manager â†’ vocalype.exe â†’ Working Set (no dictation) |
...
| `first_successful_dictation_time_ms` | 0 | 5 needed | Fresh session â€” app open â†’ first successful paste |
...
**Primary insight (H1 â€” confirmed medium-high confidence):**
...
Pipeline p50 breakdown (38 runs):
  ...
  paste_execute               645 ms   62%  â† DOMINANT
  other                        95 ms    9%
  TOTAL                      1043 ms  100%
```

Parakeet inference completes in a median of **230 ms per chunk** â€” a real-time factor
of ~0.06x. The model is fast. The bottleneck is clipboard/keyboard injection.

**Theoretical impact:**
```
If inference â†’ 0 ms:               ~740 ms  (+30% gain)
If paste     â†’ 300 ms:             ~698 ms  (+33% gain)
If paste     â†’ 100 ms:             ~450 ms  (+57% gain)  â† highest leverage
```
...
**Secondary insight (H2 â€” high confidence):**

> `chunk_cleanup` is the **tail-latency driver.** It fires in 24% of runs
> (9/38) and accounts for 100% of p95+ outliers.

```
When chunk_cleanup = 0 ms: p95 â‰ˆ 1550 ms
When chunk_cleanup fires:  p95 â†’ 2405â€“4747 ms
```
...
1. `paste_execute` â€” constant 645ms overhead, reducible without any model change
2. `chunk_cleanup` â€” 24% of runs hit tail latency; fixing trigger condition or gating
...
**Type:** `investigation_only` â€” read-only source inspection
**File:** `internal/brain/outputs/product_patch_proposal_report.md`
**JSONL:** `internal/brain/data/product_patch_proposals.jsonl` (2nd entry)

The proposal authorizes a read-only investigation of:
- `src-tauri/src/actions/paste.rs` â€” full paste mechanism
- `src-tauri/Cargo.toml` + `Cargo.lock` â€” Enigo version
- `src-tauri/src/actions/profiler.rs` â€” timing window boundaries
...
- [ ] Baseline locked (all 10 priority metrics â‰¥5 obs)
- [ ] paste_latency confirmed on â‰¥2 device profiles
...
| V8 gate | Status |
|---|---|
| â‰¥5 V7 manual benchmark sessions complete | âŒ Not met â€” 8/10 metrics at 0 obs |
| V7 baseline locked in `benchmark_baseline.jsonl` | âŒ Not met â€” requires script + â‰¥5 obs per metric |
| At least one product change benchmarked before AND after | âŒ Not met â€” no patches applied |
...
These are not optional â€” they are priority metrics that could reveal a worse problem
...
| Business metric baseline | retention_rate, activation_rate, dictations_per_week â€” from Supabase |
...
Apply paste_execute optimization â†’ re-run benchmarks â†’ compare to V7 baseline â†’ 
...
| Optimize model load time | `model_load_time_ms` is unmeasured â€” may not be a problem. |
...
**Phase: V7 Phase 1 Completion â€” Manual Metric Collection**
...
Step 2:  Open Task Manager â†’ Details tab â†’ vocalype.exe
         Record Working Set (Memory) while app is idle (no dictation).
         Record as app_idle_ram_mb.

Step 3:  Trigger a 3â€“5s dictation.
...
Step 8:  After all observations recorded:
         ...
         Check how many of the 10 priority metrics now have â‰¥5 observations.
```

### After M1 session: V7 Phase 2

Once all 10 priority metrics have â‰¥5 observations:

```
1. Build lock_benchmark_baseline.py (new script â€” not yet created)
2. Run: python internal/brain/scripts/lock_benchmark_baseline.py
3. Run: python internal/brain/scripts/lock_benchmark_baseline.py --approve
   This creates data/benchmark_baseline.jsonl â€” the locked V7 baseline.
4. Build compare_benchmarks.py (new script â€” not yet created)
```
...
Output: internal/brain/outputs/paste_mechanism_diagnosis.md
```

### After paste investigation: V8 planning

Only after:
- [ ] All 10 priority metrics at â‰¥5 observations
- [ ] Baseline locked
...
Key product insight discovered:
  ...
  Inference = 230ms median (fast â€” not the bottleneck)
  Tail latency cause: chunk_cleanup (fires 24%, up to 3320ms)

Metrics at baseline:
  total_dictation_latency_ms   âœ…  p50=1043ms  (9 obs)
  stt_inference_time_ms        âœ…  p50=230ms   (20 obs)

Metrics missing (0 obs):
  model_load_time_ms           âŒ
  app_idle_ram_mb              âŒ
  ram_during_transcription_mb  âŒ
  ram_after_transcription_mb   âŒ
  wer_percent                  âŒ
  cer_percent                  âŒ
  first_successful_dictation_time_ms  âŒ
  activation_success_rate      âŒ

Product code touched: NO (never, throughout V7)
Investigation proposals: 1 (paste_execute â€” investigation_only, pending)
Commits (brain): 7 (design â†’ scripts â†’ data â†’ reports â†’ hypothesis â†’ proposal)
