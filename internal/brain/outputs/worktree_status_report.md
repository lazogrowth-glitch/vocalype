# Vocalype Brain â€” Worktree Status Report
...
- `src/App.tsx` â€” 8 lines changed
- `src/components/auth/AuthPortal.tsx` â€” 158 lines changed
- `src/components/onboarding/FirstRunDownload.tsx` â€” 5 lines changed
- `src/hooks/useAuthFlow.test.ts` â€” 23 lines changed
- `src/hooks/useAuthFlow.ts` â€” 47 lines changed
- `src/lib/userFacingErrors.ts` â€” 19 lines changed

Note: `git diff --stat` only reports tracked file diffs, so it does not include the untracked launcher files, the new `src/lib/userFacingErrors.test.ts`, or the untracked `internal/brain/` directory.
...
- it also contains a large untracked `internal/brain/` toolchain and launcher set
