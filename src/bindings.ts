 ...
 * Does NOT record the correction â€” call `record_correction` separately.
 */
 ...
 * 
 * Safe to call multiple times â€” duplicates are skipped.
 ...
 *  only when the callback includes the expected one-time state. */
...
"document" | 
/**
 * Notes and PKM apps (Notion, Obsidian, Logseqâ€¦)
 */
"notes" | 
/**
...
"browser" | 
/**
 * App not recognised â€” neutral behaviour
 */
"unknown"
/**
 ...
 * Never set this manually â€” it is managed by `migrate_settings`.
 */
...
/**
 * Canonical recording mode â€” supersedes the `push_to_talk` and
...
/**
 * Voice snippets: short trigger phrases â†’ long expansions.
 */
...
/**
 * Pairs that have reached the auto-add threshold (â‰¥ AUTO_ADD_THRESHOLD).
 */
 ...
 * 
 * ## Migration (T11)
 * - Old `push_to_talk = true`         â†’ `RecordingMode::PushToTalk`
 * - Old `always_on_microphone = true`  â†’ `RecordingMode::AlwaysOn`
 * - Both `false` (the default)         â†’ `RecordingMode::Toggle`
 */
 ...
 * Treated as `Months3` at runtime â€” "never delete indefinitely" is not
