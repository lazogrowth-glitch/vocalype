...
Date: 2026-04-26
Source file(s) read: src-tauri/src/actions/paste.rs, src-tauri/src/actions/profiler.rs, vocalype-brain/data/benchmark_observations.jsonl
...
dispatch_text_insertion(app, operation_id, final_text, is_basic_plan, profiler, on_success)
  paste.rs:27-34   — entry point; called from transcription pipeline after model output
  paste.rs:38      — paste_time = Instant::now()  [timer A: total dispatch wall time]
  paste.rs:47-49   — profiler.set_transcription_chars(&final_text)
  paste.rs:51-56   — coordinator.mark_pasting(app, operation_id)
                     [abort if operation already cancelled/superseded]
  paste.rs:67      — app.run_on_main_thread(closure)
                     *** ASYNC BOUNDARY — calling thread returns immediately ***
                     *** closure executes when Tauri main event loop is free ***

  [Tauri main thread — executes when scheduled]
  paste.rs:70-75   — coordinator.is_operation_active(operation_id)
                     [abort if operation was cancelled while waiting for main thread]
  paste.rs:76-78   — profiler.push_step_since("paste_dispatch_wait", paste_time, None)
                     [timer A ends → records wait-for-main-thread duration]
  paste.rs:81      — paste_exec_started = Instant::now()  [timer B: paste execution wall time]
  paste.rs:84      — decide_paste_execution_mode(is_basic_plan)

  FOR NativePasteAllowed (paid tier):
  paste.rs:122     — utils::paste(final_text, app_clone)
                     *** SYNCHRONOUS — blocks main thread for full duration ***
                     *** actual OS paste mechanism — inside utils.rs (outside allowed reads) ***
  paste.rs:125-130 — profiler.push_step_since("paste_execute", paste_exec_started, Some("ok"))
                     [timer B ends → records utils::paste() duration = 644ms]
  paste.rs:131-132 — profiler.mark_completed(); profiler.emit(&app_clone)
  paste.rs:133-134 — on_success callback()
  paste.rs:136-139 — schedule_clipboard_diff_check(app_clone, final_text)
  paste.rs:141-146 — coordinator.complete_operation(&app_clone, operation_id, "pasted")

  paste.rs:191     — utils::hide_recording_overlay(&app_clone)
  paste.rs:192     — change_tray_icon(&app_clone, TrayIconState::Idle)
```

**Two independently measured timer segments exist in the code:**
- `paste_dispatch_wait` (paste.rs:77): time from `dispatch_text_insertion()` call → main thread begins
...
|---|---|---|
| `paste_dispatch_wait` | small — not isolated in logs | profiler step paste.rs:77; absent from benchmark notes suggests <20ms |
| `utils::paste()` (the `paste_execute` step) | **644ms ± 3ms** | benchmark_observations.jsonl lines 23–27: `paste_execute step from_log:...` values 687, 645, 644, 644, 644, 644, 647, 645ms |
...
**In `utils::paste()` (paste.rs:122):** Cannot confirm from allowed reads. The `utils` module is in `src-tauri/src/utils.rs` or equivalent — outside the allowed scope of this investigation. However:
...
- Standard deviation: ~13ms (nearly all spread explained by the 687ms outlier; core cluster is 644–647ms, SD ≈ 1.2ms)
- Variable OS operations (clipboard writes, window focus, keystroke simulation) would produce spread of ±50–150ms
- A 1.2ms SD is consistent with `thread::sleep(Duration::from_millis(N))` where N ≈ 600–640ms plus small OS scheduling jitter
...
- For `NativePasteAllowed`, clipboard writing is delegated to `utils::paste()` — it does NOT call `app.clipboard().write_text()` before invoking `utils::paste()`
...
**Stage 1 — `dispatch_text_insertion()` to main thread: ASYNCHRONOUS**
...
**Stage 2 — `utils::paste()` execution on main thread: SYNCHRONOUS (blocking)**
- `utils::paste(final_text, app_clone)` at paste.rs:122 is a synchronous function call
- There is no `.await`, no `spawn`, no callback — it blocks the main thread until it returns
- The `paste_execute` profiler timer (paste.rs:81 → 126) measures this synchronous block
- The Tauri main event loop is **completely blocked for ~644ms** on every paid-tier dictation

**Consequence:** During the 644ms block, no other Tauri main-thread work can execute — no UI updates, no overlay dismiss, no other command responses. The overlay hide (`utils::hide_recording_overlay`, paste.rs:191) runs only after `utils::paste()` returns.
...
**Summary:** One fallback path (native paste → clipboard copy), no retries, no timeout waits visible in paste.rs.
...
There is no shared mutex, channel, or lock between `dispatch_text_insertion()` and the inference loop visible in paste.rs. The `TranscriptionCoordinator` (paste.rs:51, 70, 108, 143, 184) manages operation lifecycle (mark_pasting, is_operation_active, complete_operation) — this is not an inference-side lock.

**Indirect coupling: Main thread contention (confirmed).**

The idle background inference loop (confirmed in benchmark_observations.jsonl line 43) fires every ~1–2 seconds, producing transcription callbacks and empty-result events. These callbacks from the inference worker thread are likely dispatched to the Tauri main thread via events.
...
2. If the inference loop fires a transcription result _during_ a paste, the result's main-thread processing waits 644ms — which could create the appearance of a double transcription trigger
...
**H1 — Remove or reduce the internal delay in `utils::paste()`.**
If a `thread::sleep(Duration::from_millis(N))` exists with N ≈ 600ms, reducing it to 0ms or a minimal value (e.g., 20ms) would directly reduce paste_latency_ms by the same amount. This would require: (a) reading utils.rs to confirm the delay exists and its purpose, (b) measuring whether removing it causes paste failures on Windows (some delays exist for clipboard propagation timing), (c) finding the minimum safe delay through bisection testing.

**H2 — Replace synchronous blocking with async paste.**
...
**H3 — Use a faster OS paste mechanism.**
...
**The most likely quick win is H1.** The ±1.2ms consistency of the benchmark cluster is the strongest indicator of a hardcoded sleep, not an inherently slow OS call. OS calls for clipboard + Ctrl+V simulation typically complete in 20–80ms on Windows; 644ms is not consistent with native API latency alone.
...
3. **Does removing or reducing the delay cause paste failures on Windows?** Some Windows paste simulation libraries require a delay for the clipboard to propagate before sending Ctrl+V. If the delay is load-bearing, the minimum safe value must be found empirically. Target: paste_latency_ms < 300ms with ≥95% paste success rate.
...
- The 644ms is caused by a fixed `thread::sleep` inside `utils::paste()` (inferred from ±1.2ms consistency; confirmed requires reading utils.rs)
