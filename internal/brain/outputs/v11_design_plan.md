# Vocalype Brain â€” V11 Design Plan
# Operating Loop â€” Weekly Action Executor
...
| Mandate | How |
|---|---|
| Read V10 weekly action | Parse `outputs/weekly_action.md` â€” extract `action_type`, `action`, evidence |
| Classify execution task | Map `action_type` â†’ Brain task classification (see Section 6) |
| Apply pre-execution safety gates | 8 gates â€” all must pass before package is written |
| Write a mission package | `outputs/v11_mission_package.md` â€” self-contained, scoped, executable |
...
| Graceful fallback on ambiguous action | Output a `data_clarification_request.md` â€” never guess scope |

### V11 phases

| Phase | Description | Gate to enter |
|---|---|---|
| Phase 1 â€” Mission package generator | Script reads weekly_action.md, runs gates, writes mission package | Now â€” PB-1 is the first target |
| Phase 2 â€” Execution recorder | Append execution results to v11_execution_log.jsonl after each mission | Phase 1 validated by founder |
| Phase 3 â€” Generalized router | Handle all 5 action types from V10 routing matrix | Phase 2 validated, â‰¥3 mission packages executed |
...
|---|---|
| Write to `src-tauri/`, `backend/`, `src/lib/auth/`, `src/lib/license/` | Permanently forbidden â€” operating contract Section 3 |
| Execute the investigation itself | V11 generates the package; a separate session executes it |
| Propose product fixes inside the mission package | Mission packages are task definitions only â€” findings go in diagnosis files |
| Skip the measurement â†’ propose â†’ implement order | Contract Section 2 â€” never compress steps |
| Write an `implementation_task` package without prior `measurement_task` output | Gate G5 â€” hard block |
| Write an `implementation_task` package without prior approved `proposal_task` output | Gate G6 â€” hard block |
...
| Produce a mission package when any safety gate fails | All 8 gates must pass â€” partial pass is a stop |
| Mark an action COMPLETE without verifying the output file exists | Gate G8 on completion â€” output file must be present before logging COMPLETE |
| Collapse multiple actions into one package | V10 emits one action per week â€” V11 maps it to one package per session |
...
|---|---|---|
| `outputs/weekly_action.md` | Source of current `action_type`, `action`, evidence | Required â€” stop if missing |
| `outputs/unified_weekly_report.md` | Layer state context (obs counts, confidence, gaps) | Required for evidence citations |
| `data/benchmark_observations.jsonl` | V7 benchmark data â€” needed for product_investigation context | Required for `product_investigation` type |
| `data/business_observations.jsonl` | V8 business data â€” needed for business_data_entry context | Required for `business_data_entry` type |
| `data/content_observations.jsonl` | V9 content data â€” needed for distribution_data_entry context | Required for `distribution_data_entry` type |
| `data/v11_execution_log.jsonl` | Existing execution records â€” check for duplicate actions | Optional â€” created on first run |
| `outputs/paste_mechanism_diagnosis.md` | Prior diagnosis output â€” required before any product_implementation | Required for `product_implementation` type |
| `outputs/handoff_task.md` (via V6) | Prior proposal output â€” required before implementation package | Required for `product_implementation` type |
| `memory/operating_contract.md` | Safety rules â€” always read before generating any package | Required |
...
| `data/v11_execution_log.jsonl` | First run (created) / every subsequent run (appended) | Immutable execution record â€” one entry per action executed |
| `outputs/v11_gate_report.md` | Every run | Gate pass/fail summary â€” replaces previous report on each run |
| `outputs/data_clarification_request.md` | Only when action is ambiguous | Written instead of mission package â€” founder must resolve before V11 re-runs |

**V11 does not overwrite `weekly_action.md` or `unified_weekly_report.md`.** Those are V10 outputs â€” V11 reads them.
...
self-contained â€” the implementation model executing it must not need to read any other
...
**Allowed reads:**
- <file or path> â€” <why it is needed>

**Allowed writes:**
- <file or path> â€” <what it must contain>

**Forbidden writes (always):**
- src-tauri/ â€” permanently forbidden
...
<evidence table extracted from weekly_action.md â€” metric, value, threshold, status>
...
<action-type-specific prohibitions â€” at least 5 items>
...
```
git add internal/brain/outputs/<output_file> internal/brain/data/v11_execution_log.jsonl
...
**Required output:** `outputs/<investigation_name>_diagnosis.md` â€” findings only, no fixes  
...
**Gate:** G3 â€” confirm target file path exists before including it in allowed reads.  
...
**V10 meaning:** A confirmed bottleneck has a diagnosis and a proposal â€” ready to implement.  
...
1. `outputs/paste_mechanism_diagnosis.md` (or equivalent diagnosis) â€” gate G5
2. `outputs/handoff_task.md` (approved V6 proposal) â€” gate G6
...
**V10 meaning:** V8 layer has insufficient data â€” founder must record business metrics.  
**V11 task classification:** `data_entry` (not implementation â€” no code)  
...
**No script execution during data_entry** â€” this is a human task, V11 writes the checklist  
...
**V10 meaning:** V9 layer has insufficient data â€” founder must record content observations.  
**V11 task classification:** `data_entry`  
**Allowed reads:** `data/content_observations.jsonl`, current V9 report  
**Required output:** Founder checklist in `outputs/v11_mission_package.md` only  
**No script execution** â€” human task  
**Commit type:** `data(brain):` (after founder records manually)  

The checklist must include:
- Publication recording command with all required flags
- Performance update timing (24â€“72h after publishing)
- Minimum posts needed before trend gate opens (â‰¥5 per platform for ranking)

---

### 6.5 `hold`

**V10 meaning:** No actionable signal this week â€” all layers below minimum data threshold.  
**V11 task classification:** `planning_only`  
**Allowed writes:** `outputs/v11_mission_package.md` (hold notice only)  
**No investigation, no data entry, no implementation** â€” explicit hold  
**Commit type:** `docs(brain):`  
...
|---|---|---|
| G1 | `outputs/weekly_action.md` exists and contains a non-empty `action_type` | Stop â€” write gate report, no package |
| G2 | `action_type` is one of the 5 supported types | Stop â€” write `data_clarification_request.md` if unknown |
| G3 | For `product_investigation`: all named read-target files exist in the repo | Stop â€” write clarification request with actual vs declared paths |
| G4 | For `product_investigation`: no write targets inside `src-tauri/`, `backend/`, `src/lib/` | Stop â€” log scope violation, no package |
| G5 | For `product_implementation`: prior diagnosis file exists (`outputs/*_diagnosis.md`) | Hard stop â€” diagnosis must precede proposal |
| G6 | For `product_implementation`: prior proposal file exists (`outputs/handoff_task.md`) | Hard stop â€” proposal must precede implementation |
| G7 | No duplicate: same `action` string not already logged as COMPLETE in `v11_execution_log.jsonl` | Warn founder â€” action may already be done; require explicit re-run flag |
| G8 | `allow_product_code_modifications` in `config/brain.config.json` is NOT `true` | Hard stop â€” safety config anomaly (operating contract S8) |

---

## 8. Stop Conditions

In addition to the operating contract's S1â€“S10, V11 adds the following stop conditions:

| # | Condition | Action |
|---|---|---|
| V11-SC1 | `weekly_action.md` does not exist | Stop â€” V10 must be run first |
| V11-SC2 | `action_type` is `hold` and the hold reason is "all layers empty" | Write hold package; do not invent a fallback task |
| V11-SC3 | Gate G3 fails (file path in action does not exist on disk) | Write clarification request â€” do not substitute a nearby file |
| V11-SC4 | Gate G5 or G6 fails for `product_implementation` | Hard stop â€” operating contract Section 2 step sequence is mandatory |
| V11-SC5 | Mission package scope would require reading >3 product files for a single diagnosis | Stop â€” scope too wide; split into sub-investigations |
| V11-SC6 | The action string from weekly_action.md cannot be parsed into a concrete read target | Write clarification request â€” ambiguous action is not guessable |
| V11-SC7 | Execution log shows the same action attempted â‰¥2 times without COMPLETE status | Stop â€” something is blocking; founder must review before next attempt |
...
reading `src-tauri/` files â€” outside the normal `internal/brain/` scope.

V11 applies a four-layer read scope discipline:

**Layer 1 â€” Named files only.** The mission package lists exact files by absolute path.
...
**Layer 2 â€” Read-only enforcement.** The mission package explicitly states:
> "No writes to src-tauri/ or any product file. This is a read-only investigation."
The output file is always inside `internal/brain/outputs/`. The implementation model
writes zero bytes to the product codebase.

**Layer 3 â€” Findings-only output.** The diagnosis file schema (see below) forbids
...
**Layer 4 â€” Controlled question set.** The mission package provides an exact list of
...
### PB-1 mission package â€” exact question set for `paste_mechanism_diagnosis.md`
...
8. What would need to change to bring paste_latency_ms below 300ms? (Hypothesis only â€” no code change in this file.)
...
<any retry loops, fallback paths, or timeout waits â€” with file:line references>

---

## Relationship to Idle Inference Loop

<whether a running inference could block the paste path â€” with evidence>
...
V11 writes mission package: "Run V6 generate_handoff_task.py for <target>"
    â†“
Implementation model runs V6 handoff â€” applies all V6 gates
    â†“
...
```json
{
  ...
  "blocked_reason": "if BLOCKED â€” why",
...
|---|---|---|
| Phase 1 | `generate_mission_package.py` â€” reads weekly_action.md, runs 8 gates, writes v11_mission_package.md + v11_gate_report.md | Now â€” PB-1 is the target |
| Phase 1 | Manual execution of PB-1 mission package â€” read paste.rs, write paste_mechanism_diagnosis.md | After Phase 1 script is committed |
| Phase 1 | Record PB-1 execution result in v11_execution_log.jsonl | After diagnosis is written |
| Phase 2 | `record_execution_result.py` â€” CLI recorder for v11_execution_log.jsonl | After Phase 1 validated |
...
| Phase 3 | Generalized router for all 5 action types | After Phase 2 validated and â‰¥3 packages executed |
| Phase 3 | `product_implementation` routing to V6 handoff | After â‰¥1 diagnosis + â‰¥1 approved proposal on file |
...
```bash
# 1. Syntax check
python -m py_compile internal/brain/scripts/generate_mission_package.py

# 2. Dry-run (gates only â€” no file writes)
python internal/brain/scripts/generate_mission_package.py --dry-run

# 3. Full run â€” generates mission package
python internal/brain/scripts/generate_mission_package.py

# 4. Verify outputs exist
ls internal/brain/outputs/v11_mission_package.md
ls internal/brain/outputs/v11_gate_report.md

# 5. Verify no product files written
git -C <repo_root> diff --name-only

# 6. Verify v11_execution_log.jsonl format
python -c "import json; [json.loads(l) for l in open('internal/brain/data/v11_execution_log.jsonl')]"

# 7. Confirm git status â€” only internal/brain/ files
git status --short
```
...
V11 closes the Brain's weekly execution cycle. Once V11 Phase 3 is live and â‰¥4 complete
...
2. **Bottleneck resolution is traceable.** When a `product_investigation` leads to a `product_implementation` that produces a new benchmark observation showing paste_latency_ms < 300ms, V12 can close the loop: action â†’ fix â†’ metric improvement.
...
4. **Hold weeks are explicit.** V11 `hold` records give V12 a clean signal that those weeks had no actionable signal â€” not silence, but an explicit hold. V12 can measure hold rate without false attribution.

**V12 entry gate:** â‰¥4 COMPLETE records in `v11_execution_log.jsonl` spanning at least two different action types.
...
- internal/brain/outputs/v11_design_plan.md
- internal/brain/outputs/weekly_action.md
- internal/brain/outputs/unified_weekly_report.md

Mission:
Implement V11 Phase 1 â€” Mission Package Generator.

Task type:
implementation_task (Brain-only â€” no product code).

Goal:
Create generate_mission_package.py that:
1. Reads outputs/weekly_action.md
2. Parses action_type, action, evidence
3. Runs 8 safety gates (G1â€“G8 from v11_design_plan.md Section 7)
...
Do not use write_jsonl â€” append to v11_execution_log.jsonl manually (append mode).

Validation:
1. python -m py_compile internal/brain/scripts/generate_mission_package.py
2. python internal/brain/scripts/generate_mission_package.py --dry-run
3. python internal/brain/scripts/generate_mission_package.py
...
6. git status --short â€” confirm only internal/brain/ files

If validation passes and only internal/brain files changed, commit with:
feat(brain): add V11 mission package generator

Rules:
- Do not modify product code.
- Do not read paste.rs yet â€” that is the mission package's target, not this script.
...
| Item | Value |
|---|---|
| V11 role | Operating Loop â€” converts V10 weekly action into safe mission package |
| Phase 1 target | `generate_mission_package.py` â†’ `v11_mission_package.md` |
| First mission | PB-1 â€” `paste_mechanism_diagnosis.md` (product_investigation, measurement_task) |
| Gate count | 8 gates â€” all must pass before package is written |
| Supported action types | 5: product_investigation, product_implementation, business_data_entry, distribution_data_entry, hold |
| Execution log | `data/v11_execution_log.jsonl` â€” append-only, one record per completed action |
| V12 entry gate | â‰¥4 COMPLETE records across â‰¥2 action types |
| Product code written | None â€” V11 is Brain-only |
| Blocked on | Nothing â€” ready to implement Phase 1 immediately |
