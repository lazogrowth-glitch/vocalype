"""apply_approved_patch.py — V3.5 controlled patch application.
...
Only vocalype-brain/ and docs/README target files are permitted.
...
def _parse_apply_instructions(patch_text: str) -> ApplyInstructions | None:
    ...
    Expected format inside the section:
        ...
        target_file: vocalype-brain/some/path.md
...
def _dry_run(
    ...
    divider = "=" * 60
    print(divider)
    print("Vocalype Brain — Apply Approved Patch (DRY RUN)")
    ...
    print(f"\nSafety class applyable : {'YES' if applyable else 'NO — ' + safety_class + ' is not in ' + str(APPLYABLE_SAFETY_CLASSES)}")
    ...
    if instructions is None:
        ...
        print(f"  target safe : {'YES — ' + target_reason if target_safe else 'NO — ' + target_reason}")

        would_apply = applyable and target_safe
        print(f"\n  Would apply with --approve : {'YES' if would_apply else 'NO'}")

    print(f"\n{divider}")
    print("To apply: python vocalype-brain/scripts/apply_approved_patch.py --approve")
    print(divider)
...
def _apply(
    ...
    # safe docs prefix — even if the string checks above passed.
    try:
        resolved.relative_to(BRAIN_ROOT)
    except ValueError:
        # Not under BRAIN_ROOT — check docs/README allowlist
        rel = str(resolved).replace("\\", "/")
        if not any(pattern in rel for pattern in APPLY_ALLOWED_PATTERNS):
            return (
                "refused",
                f"Resolved path '{resolved}' is outside vocalype-brain/ and "
...
def _write_report(
    ...
    if instructions:
        ...
        lines.append("- NOT FOUND — no structured Apply Instructions section in patch file.")
...
def main(approve: bool = False) -> None:
    ...
    print("Written vocalype-brain/outputs/apply_patch_report.md")
