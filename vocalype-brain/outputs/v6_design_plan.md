# Vocalype Brain — V6 Design Plan
# Product Implementation Handoff Loop

Date: 2026-04-24
Status: planning_only — design document only, no scripts implemented yet
...
2. **Validate** that the proposal is approved and safe to hand off (risk ≠ high, no
...
4. **Generate** a structured `handoff_task.md` — a self-contained implementation task
...
- **Never modify product code** — `src/`, `src-tauri/`, `backend/` are read-only at most.
- **Never auto-apply** any patch or diff.
- **Never infer scope** — if the approved proposal does not specify exact file paths, V6
...
- **Never commit on behalf of the implementation model** — commit is always a manual step.
- **Never use `--no-verify`**.
- **Never implement V7** — V6 must only prepare for V7 by recording benchmark metadata
...
| Measurement plan | `outputs/measure_activation_failure_points.md` | Background: failure points, options O1–O6 |
| Brain config | `config/brain.config.json` | Safety rules, forbidden patterns |
| Product target files | `src/components/auth/AuthPortal.tsx` etc. | Read-only — extract existing code context |
| Existing i18n keys | `src/locales/en.json` (or equivalent) | Read-only — avoid hardcoded strings |
| Applied patches log | `data/applied_patches.jsonl` | Deduplication — do not re-propose applied patches |
...
**`handoff_task.md`** is the primary deliverable — a complete, self-contained Markdown
...
{1–3 sentence description of the user-facing problem, sourced from proposal.summary}

## Why It Matters

{1–2 sentence impact statement, sourced from proposal.why_it_matters}
...
|---|---|---|
| G1 — Proposal exists | `product_patch_proposals.jsonl` non-empty | Abort, log: "No approved proposal" |
| G2 — Proposal status | `status == "proposed"` | Abort, log: "Proposal not in proposed state" |
| G3 — Manual approval | `manual_approval_required == true` | Abort, log: "Proposal missing manual approval flag" |
| G4 — Risk level | `risk != "high"` | Abort, log: "Risk is HIGH — founder review required" |
| G5 — Sensitive files | `sensitive_files_involved == false` | Abort, log: "Sensitive files involved" |
| G6 — Target files defined | `len(target_files) > 0` | Abort, log: "No target files defined" |
| G7 — Forbidden patterns | None of target_files matches FORBIDDEN_PATTERNS | Abort, log: "Forbidden file in scope: {file}" |
| G8 — Deduplication | Not already in `applied_patches.jsonl` | Abort, log: "Patch already applied" |
| G9 — Context readable | Target files exist and are readable | Abort, log: "Cannot read target file: {file}" |

**Belt-and-suspenders rule**: G7 is checked twice — once on the raw proposal strings, once
...
signature, existing state names, current error messages, and relevant i18n keys — it does
not need to explore the codebase and cannot accidentally widen scope.

### 7b — Explicit Forbidden Scope
...
### 7c — Stop Conditions in the Task Document Itself
...
>  Return to measure → diagnose → propose cycle before generating implementation task."
...
| `measurement_task` | Proposal source is a measurement plan option (O1–O6) that requires data collection before implementation | `handoff_task.md` with measurement checklist only |
...
### 9a — Benchmark Metadata Placeholder in Handoff Task
...
This records *which metrics matter* for this specific change — so V7 can know exactly
what to benchmark when it runs.

### 9b — Benchmark Scope Note in handoff_tasks.jsonl
...
| File | Role | Notes |
|---|---|---|
| `vocalype-brain/scripts/generate_handoff_task.py` | Main V6 script | Reads proposal, extracts context, applies gates, writes handoff_task.md |
| `vocalype-brain/scripts/review_handoff_task.py` | Review script | Reads handoff_tasks.jsonl, prints review and recommended next action |
| `vocalype-brain/scripts/brain.py` | Library | Already exists — add `read_product_file(path)` helper if needed |
| `vocalype-brain/data/handoff_tasks.jsonl` | Log | Created on first run, one record per handoff attempt |
| `vocalype-brain/outputs/handoff_task.md` | Primary deliverable | Overwritten each run with latest handoff task |
| `vocalype-brain/outputs/handoff_task_report.md` | Human review | Summary + recommended next action |
| `vocalype-brain/config/brain.config.json` | Config | Add `v6_context_max_lines` limit (default: 80 lines per file) |
| `src/components/auth/AuthPortal.tsx` | Product file — read-only | Context extracted, never written |
| `src/hooks/useAuthFlow.ts` | Product file — read-only | Context extracted, never written |

**Key constraint**: `generate_handoff_task.py` must open product files with
`open(path, 'r', encoding='utf-8')` only — never `open(path, 'w')`.
...
**V6 — Step 1: Scaffold**
1. Create `vocalype-brain/scripts/generate_handoff_task.py` with:
   - All 9 safety gates (Section 6)
   - Task classifier (Section 8)
   - `_read_product_file_safe(path)` — read-only file reader with line limit
   - `_extract_context(path)` — extract component/hook signature and relevant lines
   - `_build_handoff_task(proposal, context)` — renders `handoff_task.md` using format from Section 5
   - `_build_benchmark_scope(proposal)` — determines which V7 metrics apply
   - Dry-run mode by default, `--approve` required to write output files
   - Append to `data/handoff_tasks.jsonl` only on approve

2. Create `vocalype-brain/scripts/review_handoff_task.py` with:
...
3. Create `vocalype-brain/data/handoff_tasks.jsonl` (empty placeholder)
4. Create `vocalype-brain/outputs/handoff_task.md` (empty placeholder)
5. Create `vocalype-brain/outputs/handoff_task_report.md` (empty placeholder)
6. Update `vocalype-brain/README.md` with V6 section
7. Update `vocalype-brain/memory/current_state.md` with V6 status

**V6 — Step 2: Validate**
1. Run `python vocalype-brain/scripts/generate_handoff_task.py` (dry-run)
   → Expect: safety gates evaluated, task classified, handoff_task.md NOT written
2. Run `python vocalype-brain/scripts/generate_handoff_task.py --approve`
   → Expect: handoff_task.md written, data/handoff_tasks.jsonl appended
3. Run `python vocalype-brain/scripts/review_handoff_task.py`
   → Expect: summary printed, recommended next action shown
...
```bash
# 1. Dry-run — verify gates, no files written
python vocalype-brain/scripts/generate_handoff_task.py

# 2. Approve — write handoff task
python vocalype-brain/scripts/generate_handoff_task.py --approve

# 3. Review handoff task
python vocalype-brain/scripts/review_handoff_task.py
...
cat vocalype-brain/outputs/handoff_task.md
cat vocalype-brain/outputs/handoff_task_report.md

# 6. Confirm log appended
tail -n 1 vocalype-brain/data/handoff_tasks.jsonl

# 7. Lint Brain scripts (no product code)
python -m py_compile vocalype-brain/scripts/generate_handoff_task.py
python -m py_compile vocalype-brain/scripts/review_handoff_task.py
```
...
|---|---|---|
| S1 | No product patch proposals exist | `STOP: no product proposals found — run generate_product_patch_proposal.py first` |
| S2 | Latest proposal status is not "proposed" | `STOP: proposal status is "{status}" — only "proposed" proposals can be handed off` |
| S3 | Proposal risk is "high" | `STOP: proposal risk is HIGH — founder must narrow scope before handoff` |
| S4 | Proposal involves sensitive files | `STOP: sensitive files detected in proposal — requires explicit founder approval` |
| S5 | Target files list is empty | `STOP: no target files in proposal — return to measure → propose cycle` |
| S6 | Any target file matches FORBIDDEN_PATTERNS | `STOP: forbidden file in scope: {file} — remove from target_files and re-propose` |
| S7 | Patch already in applied_patches.jsonl | `STOP: this patch was already applied ({date}) — verify current state before re-applying` |
| S8 | Target file cannot be read | `STOP: cannot read target file {file} — verify path and permissions` |
| S9 | proposed_changes is empty or vague | `STOP: proposed_changes too vague — return to measure → diagnose → propose cycle` |
| S10 | brain.config.json missing allow_product_code_modifications | `STOP: brain config missing safety key — do not proceed` |
| S11 | brain.config.json has allow_product_code_modifications: true | `STOP: safety config anomaly — allow_product_code_modifications must be false` |
| S12 | V6 script itself would write to a product file | `STOP: internal safety violation — attempted write to product file {file}` |
...
can act on it safely". It does not implement — it prepares. Its primary value is:
...
V7 will own the "test → compare" step.
