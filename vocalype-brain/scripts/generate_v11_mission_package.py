...
def _parse_weekly_action(text: str) -> dict[str, str]:
    ...
    # Confidence — strip emoji and bold markers
    ...
    # action_type — look for **Action type:** `...`
...
class GateFailure(Exception):
    """Raised when a safety gate fails — stops package generation."""
...
def _run_gates(
    ...
    # G1 — weekly_action.md exists and has action_type
    ...
    # G3 — for product_investigation: target files must exist
    ...
    # G4 — for product_investigation: no forbidden write targets
    if action_type == "product_investigation":
        output_target = "vocalype-brain/outputs/paste_mechanism_diagnosis.md"
        forbidden_hit = next(
    ...
    # G5 — for product_implementation: prior diagnosis must exist
    ...
    # G6 — for product_implementation: prior proposal (handoff_task.md) must exist
    if action_type == "product_implementation":
    ...
    # G7 — duplicate check: same action not already COMPLETE
    ...
    _gate("G8", not allow_product,
          "allow_product_code_modifications=false — safe",
          "allow_product_code_modifications=true in brain.config.json — safety anomaly, stop.")
...
def _build_pb1_mission_package(fields: dict[str, str], evidence_rows: list[dict[str, str]]) -> str:
    ...
    return f"""# V11 Mission Package — PB-1
...
**Allowed reads:**
- `src-tauri/src/actions/paste.rs` (read-only, required) — primary investigation target{profiler_note}
...
def _build_business_data_entry_package(fields: dict[str, str]) -> str:
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    week = fields.get("week", "unknown")
    return f"""# V11 Mission Package — Business Data Entry
...
**Step 1 — Stripe**
```bash
python vocalype-brain/scripts/add_business_observation.py --metric mrr --value <VALUE> --unit usd --source stripe --period {week}
python vocalype-brain/scripts/add_business_observation.py --metric paid_conversions --value <VALUE> --unit count --source stripe --period {week}
python vocalype-brain/scripts/add_business_observation.py --metric trial_starts --value <VALUE> --unit count --source stripe --period {week}
python vocalype-brain/scripts/add_business_observation.py --metric churned_users --value <VALUE> --unit count --source stripe --period {week}
```

**Step 2 — Supabase**
```bash
python vocalype-brain/scripts/add_business_observation.py --metric account_signups --value <VALUE> --unit count --source supabase --period {week}
python vocalype-brain/scripts/add_business_observation.py --metric first_successful_dictations --value <VALUE> --unit count --source supabase --period {week}
```

**Step 3 — Vercel**
```bash
python vocalype-brain/scripts/add_business_observation.py --metric website_visitors --value <VALUE> --unit count --source vercel --period {week}
python vocalype-brain/scripts/add_business_observation.py --metric downloads --value <VALUE> --unit count --source vercel --period {week}
```
...
Minimum needed before confidence upgrades: ≥4 consecutive weeks with `downloads` + `first_successful_dictations` both recorded.
"""
...
def _build_distribution_data_entry_package(fields: dict[str, str]) -> str:
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    week = fields.get("week", "unknown")
    return f"""# V11 Mission Package — Distribution Data Entry
...
24–72h later, add performance data:
```bash
python vocalype-brain/scripts/add_content_observation.py \\
...
Minimum needed before ranking gate opens: ≥5 posts per platform.
Minimum needed before trend gate opens: ≥4 consecutive weeks.
"""
...
def _build_hold_package(fields: dict[str, str]) -> str:
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    week = fields.get("week", "unknown")
    return f"""# V11 Mission Package — Hold
...
Action: {fields.get('action', 'Explicit hold — no data sufficient for diagnosis.')}

---

## What Unlocks the Next Action

- Record V8 business metrics (Stripe, Supabase, Vercel) for ≥1 week
- Record V9 content observations for ≥1 week
- Re-run: `python vocalype-brain/scripts/generate_unified_report.py`
...
def _build_mission_package(fields: dict[str, str], evidence_rows: list[dict[str, str]]) -> str:
    ...
    # product_implementation — gates G5/G6 already confirmed files exist
    return f"""# V11 Mission Package — Product Implementation
...
Prior diagnosis: `vocalype-brain/outputs/paste_mechanism_diagnosis.md` — CONFIRMED present.
Prior proposal: `vocalype-brain/outputs/handoff_task.md` — CONFIRMED present.
...
def _build_gate_report(
    ...
    if package_written:
        lines += [
            ...
            "",
            f"Mission package written: `vocalype-brain/outputs/v11_mission_package.md`",
            "",
        ...
        lines += [
            f"**BLOCKED** — gate failure prevented package generation.",
            "",
    ...
    lines += [
        ...
        f"- Mission package written: {'yes — `vocalype-brain/outputs/v11_mission_package.md`' if package_written else 'no — gate failure'}",
        f"- Safety verdict: {'SAFE TO SEND' if package_written else 'BLOCKED'}",
        f"- Product code touched: no",
        f"- Safe to send to Claude/Codex/Aider: {'yes' if package_written else 'no — resolve gate failure first'}",
    ]
...
def _log_entry(fields: dict[str, str], package_path: str) -> dict[str, Any]:
    ...
    output_map = {
        "product_investigation": "vocalype-brain/outputs/paste_mechanism_diagnosis.md",
        "business_data_entry": "vocalype-brain/data/business_observations.jsonl",
        "distribution_data_entry": "vocalype-brain/data/content_observations.jsonl",
    }
    return {
        ...
        "source_file": "vocalype-brain/outputs/weekly_action.md",
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true",
                        help="Run gates only — do not write any files")
    ...
    try:
        ...
        # gate_results may be partial — fill rest as skipped
    ...
    print(f"\nWritten: vocalype-brain/outputs/v11_mission_package_report.md")
    ...
    print(f"Written: vocalype-brain/outputs/v11_mission_package.md")

    # Append PENDING log entry
    entry = _log_entry(fields, "vocalype-brain/outputs/v11_mission_package.md")
    append_jsonl("data/v11_execution_log.jsonl", entry)
    print(f"Appended: vocalype-brain/data/v11_execution_log.jsonl (execution_id={entry['execution_id']}, status=PENDING)")
