...
|---|---|---|
| 1 | First successful dictation | North Star activation event — blocks all downstream value |
...
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 1 — Activation                                            │
│  User opens app → auth/license check → status reaches "ready"  │
│  Measured: time from app launch to first "ready" state          │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 2 — Trigger                                               │
│  User triggers dictation (hotkey / overlay click)               │
│  Measured: time from trigger to first audio capture frame       │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 3 — Audio Capture                                         │
│  Microphone open → VAD detection → silence end detection        │
│  Measured: capture duration, VAD confidence, silence threshold  │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 4 — Preprocessing                                         │
│  Raw audio → resampling → normalization → model input format    │
│  Measured: preprocessing wall-clock time                        │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 5 — Model Load / Warmup                                   │
│  Is model already loaded? First-run load vs. warm inference     │
│  Measured: cold load time, warm inference ready time            │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 6 — STT Inference                                         │
│  Audio tensor → Parakeet model → raw token output              │
│  Measured: inference wall-clock time, real-time factor (RTF)    │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 7 — Post-processing                                       │
│  Tokens → text → punctuation → formatting                       │
│  Measured: post-processing wall-clock time                      │
└──────────────────────────────┬──────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 8 — Output / Paste                                        │
│  Text → clipboard injection or native paste                     │
│  Measured: paste success/failure, injection latency             │
└─────────────────────────────────────────────────────────────────┘

TOTAL PERCEIVED LATENCY = trigger → paste complete
...
### 3A — Transcription Latency

| Metric | Unit | Description |
|---|---|---|
| `trigger_to_capture_ms` | ms | Hotkey/click → first audio frame |
| `capture_duration_ms` | ms | Duration of captured audio |
| `preprocessing_ms` | ms | Audio → model input |
...
| `postprocessing_ms` | ms | Token → final text |
| `paste_ms` | ms | Text → output injected |
| `total_perceived_latency_ms` | ms | trigger → paste complete (primary metric) |
...
WER = Word Error Rate = (substitutions + deletions + insertions) / reference words × 100

### 3D — Activation / Stability

| Metric | Unit | Description |
|---|---|---|
| `activation_success_rate` | % | Sessions reaching "ready" / total sessions |
| `activation_time_ms` | ms | App launch → first "ready" state |
...
| `first_dictation_total_time_ms` | ms | App install → first successful paste |
...
|---|---|---|
| M1 | Cold start → activation → first dictation | activation_time_ms, first_dictation_total_time_ms |
| M2 | Warm start → dictation → paste | trigger_to_paste_ms, inference_ms |
| M3 | Short French phrase (5–8 words) | wer_french_short |
| M4 | Short English phrase (5–8 words) | wer_english_short |
...
```
benchmark_runner.py
  ├── reads: data/benchmark_config.json (test cases, reference texts)
  ├── controls: app via Tauri CLI flags or IPC (read-only, no product code change)
  ├── records: data/benchmark_results.jsonl (one record per test)
  ├── reports: outputs/benchmark_report.md
  └── compares: against baseline in data/benchmark_baseline.jsonl
```
...
| Benchmark config | `vocalype-brain/data/benchmark_config.json` | Test cases, thresholds (V7 Phase 2) |
| Manual observations | `vocalype-brain/data/benchmark_observations.jsonl` | Founder-recorded sessions |
| Baseline snapshot | `vocalype-brain/data/benchmark_baseline.jsonl` | Locked baseline for comparison |
| Applied patches log | `vocalype-brain/data/applied_patches.jsonl` | Link benchmark to product change |
| Handoff tasks log | `vocalype-brain/data/handoff_tasks.jsonl` | V6 benchmark_scope field drives which tests to run |
...
|---|---|---|
| Manual observations | `vocalype-brain/data/benchmark_observations.jsonl` | Founder (manual) |
| Automatic results | `vocalype-brain/data/benchmark_results.jsonl` | `benchmark_runner.py` (Phase 2) |
| Baseline snapshot | `vocalype-brain/data/benchmark_baseline.jsonl` | `lock_benchmark_baseline.py` (Phase 2) |
| Benchmark report | `vocalype-brain/outputs/benchmark_report.md` | `review_benchmarks.py` |
| Comparison report | `vocalype-brain/outputs/benchmark_comparison.md` | `compare_benchmarks.py` (Phase 2) |
...
crash, notes
```
All timing fields are integers (milliseconds). All rate fields are floats (0.0–1.0).
Unknown values use `null`, not `0` or `""`.

### `benchmark_baseline.jsonl`
One record per metric. Locked by the founder after ≥5 manual sessions.
```json
{
...
|---|---|---|
| G1 — Read-only | Benchmark scripts never write to `src/`, `src-tauri/`, `backend/` | Abort — log safety violation |
| G2 — No instrumentation | Benchmark runner does not modify product source to add timing hooks | Stop — flag as separate task |
| G3 — No model side-effects | Benchmark does not change model weights, config, or runtime state | Abort |
| G4 — No auth/license writes | Benchmark does not store credentials, tokens, or license data | Abort |
| G5 — Baseline locked | Comparison reports require a locked baseline; refuse if missing | Warn — run Phase 1 first |
| G6 — App version tagged | Every result record must include the `app_version` (git SHA) | Warn — record is unreliable |
| G7 — Observer identified | Manual records must identify `observer: "founder"` or named person | Warn — data provenance unclear |
...
| # | Condition | Action |
|---|---|---|
| S1 | No manual observations exist yet | Stop Phase 2 — complete ≥5 manual sessions first |
| S2 | Baseline not locked | Stop comparison reports — `lock_benchmark_baseline.py` must run first |
| S3 | Benchmark script would write to product files | Abort — log refusal |
| S4 | App version unknown (no git SHA) | Warn and record `app_version: "unknown"` — do not abort, but flag |
| S5 | WER test phrase and reference phrase are identical | Stop — trivial test, not meaningful |
| S6 | RAM delta per dictation > 50 MB across 5 sessions | Flag as memory leak — do not optimize until diagnosed |
| S7 | crash_rate_per_100_sessions > 5 | Stop other benchmark phases — stability must be fixed first |
| S8 | Benchmark runner requests product file write | Abort — gate G1 violation |
| S9 | Founder requests automated benchmark before manual baseline exists | Stop — manual first, automated second |
...
### V7 Phase 1 — Manual Baseline (no scripts needed)

1. Founder runs ≥5 manual benchmark sessions using the M1–M7 test protocol (Section 4).
2. Records are appended to `data/benchmark_observations.jsonl` manually or via a simple append helper.
3. After ≥5 sessions, build `review_benchmarks.py`:
   - Reads `data/benchmark_observations.jsonl`
   - Computes p50/p95 for each metric
   - Writes `outputs/benchmark_report.md`
   - No product code touched

### V7 Phase 2 — Baseline Lock and Comparison (after Phase 1)
...
### V7 Phase 3 — Automated Runner (after Phase 2, separate planning_only task)

6. Design `benchmark_runner.py` — uses existing CLI/IPC interfaces only, no product instrumentation.
...
- `vocalype-brain/data/benchmark_observations.jsonl` (empty)
- `vocalype-brain/data/benchmark_baseline.jsonl` (empty)
- `vocalype-brain/data/benchmark_results.jsonl` (empty)
- `vocalype-brain/data/benchmark_config.json` (thresholds config)
- `vocalype-brain/outputs/benchmark_report.md` (empty placeholder)
...
```bash
# After Phase 1 — review manual observations
python vocalype-brain/scripts/review_benchmarks.py

# After Phase 1 — lock baseline (requires --approve)
python vocalype-brain/scripts/lock_benchmark_baseline.py
python vocalype-brain/scripts/lock_benchmark_baseline.py --approve

# After Phase 2 — compare new results to baseline
python vocalype-brain/scripts/compare_benchmarks.py

# Compile check (after scripts are built)
python -m py_compile vocalype-brain/scripts/review_benchmarks.py
python -m py_compile vocalype-brain/scripts/lock_benchmark_baseline.py
python -m py_compile vocalype-brain/scripts/compare_benchmarks.py
...
cat vocalype-brain/outputs/benchmark_report.md
cat vocalype-brain/outputs/benchmark_comparison.md
```
...
### 13a — Benchmark scope field in handoff records
...
by matching git SHA → deploy date → conversion/retention window.

### 13c — Metric taxonomy alignment
...
### 13d — What V8 must NOT inherit from V7
...
### 13e — Stop condition for V8
V8 must not be designed until:
1. ≥5 V7 manual benchmark sessions are complete.
...
**Critical path:**
1. Run ≥5 manual sessions (Section 4, M1–M7 protocol)
