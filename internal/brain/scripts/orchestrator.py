...
def run_daily() -> None:
    ...
    print(f"Generated daily report using {mode}: internal/brain/outputs/daily_actions.md")
...
def run_growth() -> None:
    ...
    print("Generated internal/brain/outputs/growth_report.md")
...
def run_product() -> None:
    ...
    for action in actions[:5]:
        save_action(action)
    score_actions_tool()
    print(f"Saved {min(len(actions), 5)} product actions to internal/brain/data/actions.jsonl")


def run_self_improve() -> None:
    proposals = generate_improvement_proposals()
    print(f"Generated {len(proposals)} self-improvement proposals: internal/brain/outputs/improvement_proposals.md")
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            "Usage: python internal/brain/scripts/orchestrator.py "
    ...
    if command == "daily":
        ...
        if len(sys.argv) < 3:
            raise SystemExit('Usage: python internal/brain/scripts/orchestrator.py ask "What should we improve today?"')
