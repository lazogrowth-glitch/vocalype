...
def _classify_safety(files: list[str], task_type: str) -> tuple[str, str]:
    ...
    safety_class is one of:
      brain_safe           â€” only internal/brain/ files
      docs_safe            â€” only README/docs/markdown files
      product_proposal_only â€” product files in scope but not forbidden
      unsafe               â€” forbidden scope detected
    """
    if not files:
        if task_type in ("planning_only", "measurement_task"):
            return (
                ...
                "changes are limited to internal/brain/ outputs.",
            )
        return (
    ...
    # All files inside internal/brain/
    brain_only = all(BRAIN_PREFIX in f or f.startswith("internal/brain/") for f in files)
    if brain_only:
        return "brain_safe", "All target files are inside internal/brain/."

    # All non-Brain files are docs-safe
    non_brain = [f for f in files if BRAIN_PREFIX not in f and not f.startswith("internal/brain/")]
    ...
    # Product files in scope â€” check against safe set
    ...
    if outside_safe:
        ...
        return (
            ...
            "Proposal only â€” requires manual inspection before application.",
        )
...
def _patch_file_content(
    ...
    lines = [
        ...
        f"Manual approval required: {'YES â€” do not apply without founder review' if is_proposal else 'Yes (review before applying)'}",
        "",
        "Target files:",
    ]
    for f in target_files:
        lines.append(f"- {f}")
    if not target_files:
        lines.append("- (none â€” Brain/docs files only)")
...
def _write_report(
    ...
    lines = [
        "# Vocalype Brain â€” Safe Patch Report",
        "",
...
def main() -> None:
    ...
    patch_path = f"internal/brain/patches/{patch_filename}"

    if safety_class == "unsafe":
        patch_file_written = ""
        status = "rejected"
        print(f"REJECTED â€” unsafe task.")
    ...
    print("Written internal/brain/outputs/safe_patch_report.md")
