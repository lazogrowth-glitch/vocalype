...
**Status:** Proposed â€” awaiting founder approval to begin investigation
...
Meanwhile, Parakeet inference completes in a **median of 230ms per chunk** â€” faster than real time
...
to ~450ms â€” a **57% improvement in user-perceived post-recording latency** with no model change.
...
| `paste_execute` variance | Â±35 ms | Very narrow â€” not noise, not I/O wait |
...
`app.run_on_main_thread()` callback â€” inside `paste_exec_started` to completion.

This implies the 645ms is NOT an async queue wait â€” it is the actual paste execution time
on the main thread.
...
| File | Purpose |
|---|---|
| `src-tauri/src/actions/paste.rs` | Full paste execution logic â€” Enigo calls, clipboard write, timing |
...
| `src/lib/auth/client.ts` | Auth client â€” security boundary |
| `src/lib/license/client.ts` | License client â€” billing boundary |
| `src-tauri/src/managers/` â€” writes | Read only in this phase |
...
## 5. Candidate Files â€” Next Implementation Handoff
...
> âš ï¸ These are hypotheses only. Do NOT implement any of these until:
...
| Use `NativePasteAllowed` path for all users | If `ClipboardOnlyBasic` is slower than `NativePasteAllowed` | Unknown â€” needs investigation | Plan enforcement logic must be preserved |
...
1. Run â‰¥10 dictations with modified paste path.
...
- [ ] Baseline is locked: all 10 priority metrics have â‰¥5 observations
- [ ] `paste_latency_ms` confirmed on at least 2 device profiles
- [ ] `paste_success_rate` has been measured (currently unmeasured)
- [ ] chunk_cleanup trigger condition is understood (H2 â€” tail latency driver)
...
Output: paste_mechanism_diagnosis.md â€” answers to the 6 questions above, with file:line citations.
