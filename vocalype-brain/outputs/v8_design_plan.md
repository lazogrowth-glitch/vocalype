# Vocalype Brain — V8 Design Plan
# Business Metrics Visibility Loop

Date: 2026-04-25
Task type: planning_only
Status: design document only — no scripts implemented yet
...
| Surface funnel drop-offs | Show where users are lost: download → install → activate → dictate → convert |
| Track North Star metric | `first_successful_dictation_count` per week — the only metric that matters at activation stage |
| Flag anomalies | RAM growth, MRR drop, conversion collapse — surface these before they become crises |
...
| Auto-post content to TikTok, Twitter, LinkedIn | V9 scope — not V8 |
...
| Track individual user behaviour or PII | Aggregate metrics only — no user-level data in Brain files |
...
| `first_successful_dictation_week` | count | Supabase history | **North Star** — users who completed first dictation |
...
### 3C — Engagement / Retention

| Metric | Unit | Source | Notes |
|---|---|---|---|
| `dau` | count | Supabase | Daily active users (any dictation) |
| `wau` | count | Supabase | Weekly active users |
| `dictations_per_wau_week` | float | Supabase | North Star numerator / WAU — usage intensity |
...
| `paid_conversions_week` | count | Stripe | Trial → paid this week |
...
### 3E — Founder Actions (accountability layer)
...
The join key is `week_of` (ISO week) — both V7 product observations and V8 business
...
The Brain will never declare "latency fix caused conversion increase" — it will report
...
1. Open Vercel Analytics → record website_visitors_week, download_page_views_week
2. Open Stripe Dashboard → record mrr_usd, paid_conversions_week, trial_starts_week, churn_count_week
3. Open Supabase → auth.users, history table → record new_app_installs_week, first_successful_dictation_week
4. Open TikTok / social analytics → record content_views_week
5. Count manually: content_posts_published_week, outreach_contacts_week, user_interviews_week
6. Record each using add_business_observation.py (V8 Phase 1 script — to be built)
7. Run review_business_metrics.py → read outputs/business_metrics_report.md
```
...
- Must never store user-level data — aggregate only
...
| Input | Path | Purpose |
|---|---|---|
| Brain config | `vocalype-brain/config/brain.config.json` | Safety rules |
| Business observations | `vocalype-brain/data/business_observations.jsonl` | Weekly metric records |
| Business baseline | `vocalype-brain/data/business_baseline.jsonl` | Locked baseline for comparison |
| V7 benchmark observations | `vocalype-brain/data/benchmark_observations.jsonl` | Product metric join data |
| Applied patches log | `vocalype-brain/data/applied_patches.jsonl` | Link business delta to product change |
| Handoff tasks log | `vocalype-brain/data/handoff_tasks.jsonl` | Which product change shipped when |
| Operating contract | `vocalype-brain/memory/operating_contract.md` | Safety rules |
| Current state | `vocalype-brain/memory/current_state.md` | Phase tracking |
...
|---|---|---|
| Business observations | `vocalype-brain/data/business_observations.jsonl` | `add_business_observation.py` (manual entry) |
| Business baseline | `vocalype-brain/data/business_baseline.jsonl` | `lock_business_baseline.py` (Phase 2) |
| Weekly business report | `vocalype-brain/outputs/business_metrics_report.md` | `review_business_metrics.py` |
| Product–business correlation | `vocalype-brain/outputs/product_business_correlation.md` | `correlate_metrics.py` (Phase 2) |
| V8 status report | `vocalype-brain/outputs/v8_status_report.md` | Manual (at phase close) |

---

## 9. Proposed JSONL Schemas

### `business_observations.jsonl` — one record per metric per week
...
**Field rules:**
- `week_of`: ISO week string (`YYYY-Www`) — standard grouping key
- `metric`: snake_case, from the approved metric list in Section 3
- `value`: always numeric — no strings, no nulls (omit record if unknown)
...
### `business_baseline.jsonl` — one record per metric (locked)
...
### `product_business_correlation.jsonl` — one record per product change + business delta
...
```json
{
  ...
  "notes": "correlation only — confounders: content post published same week"
}
```
...
|---|---|---|
| G1 — Read-only Brain | Scripts never write to `src/`, `src-tauri/`, `backend/` | Abort — log safety violation |
| G2 — Aggregate only | No user-level PII in any Brain file | Abort — flag and redact |
| G3 — No API secrets in repo | Credentials must come from env vars, never from Brain files | Abort — do not commit |
| G4 — Manual before automated | Phase 2 automation requires ≥4 weeks of manual baseline | Stop — run Phase 1 first |
| G5 — Baseline locked before comparison | `compare_business_metrics.py` refuses if baseline missing | Warn — lock baseline first |
| G6 — App version tagged | Every observation record must include `app_version` | Warn — record is unreliable |
| G7 — Week string validated | `week_of` must be valid ISO week format | Error — reject malformed record |
| G8 — Correlation ≠ causation | Reports must label correlations explicitly, never claim causation | Manual check before committing |
| G9 — Founder records source | Every manual observation must have a `source` field | Warn — data provenance unclear |
...
| # | Condition | Action |
|---|---|---|
| S1 | Metric requires reading production database directly | Stop — export CSV manually instead |
| S2 | Observation contains individual user data (email, name, IP) | Stop — redact, use aggregate only |
| S3 | Script would write to product files | Abort — gate G1 violation |
| S4 | Business metric change cannot be explained (no product change, no campaign) | Flag as anomaly — do not declare win or loss |
| S5 | MRR drops >20% week-over-week | Stop all V8 optimisation work — investigate cause first |
| S6 | `churn_count_week` > `paid_conversions_week` for 3 consecutive weeks | Flag as churn crisis — escalate to founder |
| S7 | `first_successful_dictation_week` = 0 for any week | Stop — activation is broken, fix before measuring anything else |
| S8 | Correlation report shows product metric improved but business metric worsened | Do not declare win — investigate confounders before next patch |
| S9 | V8 Phase 2 API automation requested before 4 weeks of manual baseline | Stop — Phase 1 first |
| S10 | Founder asks V8 to post content, run ads, or modify backend pricing | Stop — that is V9 or forbidden scope |

---

## 12. Future Implementation Steps

### V8 Phase 1 — Manual Weekly Cadence (no API calls)
...
Step 3: Founder runs weekly 10-minute session
  - Record 8–12 metrics from Stripe / Supabase / Vercel dashboards
  - Run review_business_metrics.py
  - Read report — note any anomalies
...
### V8 Phase 2 — Automated Pulls (after Phase 1 baseline)
...
Step 7: Scheduled weekly run (cron or manual trigger)
  - fetch_business_metrics.py → review_business_metrics.py → correlate_metrics.py
  - Output: business_metrics_report.md + product_business_correlation.md
  - Brain commits outputs (docs commit, brain files only)
```

### V8 Phase 3 — Signal Threshold Alerts (after Phase 2)
...
Step 8: Build alert_thresholds.py
  ...
  - Does NOT auto-escalate — founder reads alerts manually
```
...
```bash
# After Phase 1 scripts are built — compile check
python -m py_compile vocalype-brain/scripts/add_business_observation.py
python -m py_compile vocalype-brain/scripts/review_business_metrics.py

# After first manual session — verify observation recorded
python vocalype-brain/scripts/review_business_metrics.py
cat vocalype-brain/outputs/business_metrics_report.md

# After Phase 1 baseline (4 weeks) — lock baseline
python vocalype-brain/scripts/lock_business_baseline.py
python vocalype-brain/scripts/lock_business_baseline.py --approve
...
cat vocalype-brain/outputs/business_metrics_report.md
cat vocalype-brain/outputs/product_business_correlation.md

# Verify no PII in observations
grep -i "email\|@\|phone\|ip_addr" vocalype-brain/data/business_observations.jsonl
...
V9 is not automation for its own sake — it is automation of what V8 proves works.

V8 prepares V9 by:

### 14a — Identifying the highest-leverage distribution channel

After 4+ weeks of data, V8 can answer:
- Which content type drives the most download-attributed views? (TikTok vs other)
- What is the content → download → activation conversion rate?
...
### 14b — Identifying the activation bottleneck

V8 measures `first_dictation_success_rate = first_dictation / new_installs`.
If this rate is < 50%, the primary growth lever is fixing activation — not distribution.
...
### 14c — Connecting product changes to business outcomes
...
### 14d — What V9 must NOT inherit from V8

V9 must not assume:
- Content views → downloads is linear (it is not)
...
V9 requires: V8 baseline locked + ≥1 confirmed product win in product+business terms.
...
Mission:
Build V8 Phase 1 — Manual Business Metrics Recorder.
...
Build the two V8 Phase 1 scripts defined in v8_design_plan.md:
  1. vocalype-brain/scripts/add_business_observation.py
  2. vocalype-brain/scripts/review_business_metrics.py
...
Rules:
- Only write to vocalype-brain/scripts/ and vocalype-brain/data/ and vocalype-brain/outputs/
...
North Star metric:
  ...
  — the only metric that matters at this activation stage
...
V8 gates for V9:
  ...
  - ≥1 confirmed product win (product + business delta positive)
