# Vocalype Brain â€” V9 Closure Report
...
|---|---|---|
| `v9_design_plan.md` â€” 16-section distribution metrics architecture | âœ… Done | Planning only |
| `add_content_observation.py` â€” CLI recorder with publication + performance support | âœ… Done | Validated, committed |
| `review_content_performance.py` â€” report generator with platform/type/hook breakdown | âœ… Done | Validated, committed |
| `weekly_content_snapshot.py` â€” founder-facing weekly checklist | âœ… Done | Validated, committed |
| `content_observations.jsonl` â€” append-only JSONL store | âœ… Done | 1 validation sample (excluded from reports) |
| `content_report.md` â€” auto-generated performance report | âœ… Done | Regenerates on every review run |
| `weekly_content_snapshot.md` â€” founder checklist with anti-hype section | âœ… Done | Excludes validation samples correctly |
| Platform enum (5 platforms) | âœ… Done | tiktok, instagram_reels, youtube_shorts, youtube, twitter_x |
| Content type enum (8 types) | âœ… Done | demo, tutorial, pain_point, testimonial, hook_test, before_after, day_in_life, reaction |
| Niche enum (8 niches) | âœ… Done | productivity, developer, writer, student, accessibility, remote_worker, entrepreneur, general |
| Safety gates G1â€“G8 implemented | âœ… Done | No ranking < 5 posts, no trend < 4 weeks, validation excluded |
| Stop conditions SC1, SC3, SC4, SC8 implemented | âœ… Done | Views > 500K flagged, anti-trend warning |
| 4 weeks of real content baseline | âŒ Not collected | No real observations recorded yet |
| `correlate_content_business.py` â€” Phase 2 join script | âŒ Not built | Requires â‰¥10 real posts + â‰¥4 V8 weeks |
| `compare_content_experiments.py` â€” Phase 3 comparison script | âŒ Not designed | Phase 3 â€” after joint data baseline |

**Why closing without real content data:**

V9 was always a two-part phase:
1. Build the measurement infrastructure (done)
2. Feed it real content observations over time (ongoing â€” not a Brain task, a founder task)
...
The remaining V9 work â€” recording each piece of content after posting â€” is a recurring
...
| Record performance metrics 24â€“72h later | Same script with `--views`, `--likes`, `--saves`, `--lesson`, `--next_action` |
| Update an existing post with performance data | `--post_id <id> --record_type performance_update` |
| Generate weekly content performance report | `review_content_performance.py` â†’ `content_report.md` |
| Generate founder posting checklist | `weekly_content_snapshot.py` â†’ `weekly_content_snapshot.md` |
...
| Rank content types by performance | < 5 posts per type (gate G1) | After â‰¥5 posts per content_type |
| Recommend platform prioritization | No real cross-platform data | After â‰¥5 posts per platform |
...
| Automate content posting | Permanently out of V9 Phase 1â€“3 scope | Phase 4 â€” separate design |
| Pull analytics via platform APIs | No API integration | Phase 4 â€” separate design |
| Identify which hook style drives the most downloads | No V8 baseline + no content baseline | After V9 Phase 2 |
| Feed V10 with distribution layer data | No real data â†’ no signal to feed | After 4 weeks of real V9 data |
...
### 4a â€” Validation sample exclusion
...
### 4b â€” No ranking before minimum data
...
raw counts only: "3 posts â€” count only." This prevents "demo posts are better" conclusions
from 1 demo and 0 tutorials.

### 4c â€” No trend before 4 weeks
`weekly_content_snapshot.py` explicitly counts weeks of data and states "N week(s) of data â€”
baseline requires â‰¥4 weeks before any pattern is meaningful." Until 4 weeks of consecutive
posting exist, no trend language is used anywhere in the reports.

### 4d â€” downloads_attributed never auto-populated
...
### 4e â€” Anti-hype section in every snapshot
The weekly snapshot includes an explicit "Do Not Overreact Yet" section that fires
stage-appropriate reality checks: 0 views â‰  product failure, low early reach is normal
...
The correct next action is not a Brain task â€” it is the founder publishing a piece of
content, then opening the platform analytics 24â€“72h later and recording what is actually
...
**Phase 2 gate:** â‰¥10 real V9 content observations AND â‰¥4 weeks of real V8 observations.
...
|---|---|
| `paste_execute = 645ms` constant | Demo content shows a real paste delay â€” the founder should know this is a product limitation, not a recording artifact |
| Idle background inference loop (+110MB/15min) | Long recording demos risk showing RAM warnings on lower-spec machines â€” avoid extended idle periods in demo cuts |
| p95 tail latency = 2405ms (chunk_cleanup fires 24% of runs) | Demo recordings will occasionally show a slow outlier â€” not representative; retake if it appears on screen |
| `stt_inference_time = 230ms` (not the bottleneck) | Inference is fast â€” demos can honestly highlight "instant recognition" for the audio-to-text step |
...
|---|---|---|
| V7 Phase 2 baseline locked (product benchmark comparison running) | âŒ Not locked | Not waived â€” product baseline required for cross-layer correlation |
| V8 â‰¥4 weeks of real business observations | âŒ 0 weeks | Not waived â€” business layer must have real data before unified decision engine |
| V9 â‰¥4 weeks of real content observations across â‰¥2 platforms | âŒ 0 weeks | Not waived â€” distribution layer must have real data before unified decision engine |
| â‰¥1 confirmed product improvement applied and measured | âŒ None | Not waived â€” V10 needs at least one change-and-measure cycle to make useful recommendations |
| Founder sign-off on V9 Phase 2 correlation report | âŒ Not run | Not waived â€” cross-layer correlation must be validated before automated recommendations |

**Why V10 design can still begin now:**

The design of V10 does not require real data â€” it requires knowing what V10 should
...
**V10 cannot run until** all three layers (V7, V8, V9) have â‰¥4 weeks of real data
...
**V10 = Unified Decision Engine â€” Cross-Layer Weekly Recommendation**
...
> distribution reach (V9) â€” what is the highest-leverage action this week?"

V10 structure:

| Phase | Description | Gate to enter |
|---|---|---|
| V10 Phase 1 â€” Cross-layer join | Joins V7 + V8 + V9 by period. Produces a unified weekly table. | â‰¥4 real weeks in all three datasets |
| V10 Phase 2 â€” Bottleneck diagnosis | Identifies which layer is the current constraint: product, funnel, or distribution | After Phase 1 run validated by founder |
| V10 Phase 3 â€” Weekly recommendation | Outputs one ranked action per week with evidence from all three layers | After Phase 2 validated by founder |

V10 inputs:
- `data/benchmark_observations.jsonl` â€” V7 product metrics
- `data/business_observations.jsonl` â€” V8 business metrics
- `data/content_observations.jsonl` â€” V9 distribution metrics
- `memory/operating_contract.md` â€” safety rules

V10 outputs:
- `outputs/unified_weekly_report.md` â€” cross-layer join table + bottleneck diagnosis
- `outputs/weekly_action.md` â€” single ranked action with evidence (one per week)
...
- "Product: paste latency 645ms â€” 3 weeks without a fix. Funnel: 0 first dictations / 12 downloads. Priority = fix paste before adding more distribution."
...
|---|---|
| Make decisions autonomously without founder review | V10 outputs recommendations â€” the founder decides |
| Run without all three layers having real data | Joining empty datasets produces noise, not signal |
| Recommend paid advertising | No CAC/LTV baseline â€” paid cannot be justified |
...
| Declare a bottleneck from 1 week of data | One week is not a pattern â€” minimum 2 consecutive same-layer flags before bottleneck declared |
...
| Claim product-market fit, growth trajectory, or virality | Anti-hype â€” same discipline as V8 and V9 |
...
- internal/brain/outputs/v7_closure_report.md
- internal/brain/outputs/v8_closure_report.md
- internal/brain/outputs/v9_closure_report.md

Mission:
Design V10 â€” Unified Decision Engine.
...
V7 produces: benchmark_observations.jsonl
  â€” latency (paste_execute=645ms bottleneck), RAM (+110MB idle), WER
V8 produces: business_observations.jsonl
  â€” downloads, signups, first_successful_dictations, MRR, activation_attempts
V9 produces: content_observations.jsonl
  â€” platform, content_type, hook, views, profile_visits, website_clicks, lesson_learned
...
Current data state:
  ...
  Joint data: not yet possible â€” V8 and V9 need real data first
...
- Design for the future state where all three layers have â‰¥4 weeks of real data.
```
...
Verdict:   CLOSED â€” infrastructure complete, real content entry pending
...
Phase 2:   Not started (correlate_content_business.py not built â€”
           requires â‰¥10 real posts + â‰¥4 real V8 weeks)
Phase 3:   Not designed (compare_content_experiments.py â€” Phase 3)
Real data: 0 observations (founder has not posted and recorded yet)

What V9 built:
  add_content_observation.py       â€” publication + performance recorder
  review_content_performance.py    â€” platform/type/hook/lesson/backlog report
  weekly_content_snapshot.py       â€” founder checklist (anti-hype)
  v9_design_plan.md                â€” 16-section distribution architecture

Anti-traction mechanisms:
  - validation samples excluded from all reports
  - no ranking before â‰¥5 posts per type (gate G1/G2)
  - no trend before â‰¥4 weeks (gate G6)
...
V10 gates (NOT yet met):
  ...
  - V8 â‰¥4 weeks real data
  - V9 â‰¥4 weeks real data across â‰¥2 platforms
  - â‰¥1 confirmed product improvement measured
...
*All source data: `internal/brain/data/content_observations.jsonl` (validation sample only at closure).*
