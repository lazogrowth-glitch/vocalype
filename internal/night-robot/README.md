...
8. Judges before/after â€” accepts only if tests pass and benchmark improves
...
# Plan only â€” analyze and propose, no changes applied
...
Edit `internal/night-robot/config/robot_config.json`.
...
docs/infrastructure changes â€” it will reject behavior changes.
...
```
internal/night-robot/
  config/
    robot_config.json
  data/
    runs/          â† benchmark JSON
    memory/        â† lessons.jsonl
    reports/       â† per-cycle Markdown reports
    patches/       â† proposed diffs
  prompts/
    system_prompt.md
  ...
  scripts/
    night_robot.py       â† main entry point
    llm_client.py        â† httpx LLM caller
    repo_guard.py        â† forbidden path enforcement
    repo_scanner.py      â† transcription file discovery
    benchmark_runner.py  â† command runner + result saver
    patch_planner.py     â† LLM analysis + proposal
    patch_applier.py     â† git apply wrapper
    result_judge.py      â† before/after comparison
    memory_store.py      â† lessons.jsonl CRUD
    utils.py             â† timestamp, run_command, logging
```
