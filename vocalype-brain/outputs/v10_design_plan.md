# Vocalype Brain — V10 Design Plan
# Unified Decision Engine — Cross-Layer Weekly Report
...
rule-based diagnosis — no machine learning, no inference, no speculation.
...
| Run bottleneck diagnosis when ≥2 layers have ≥1 week of real data | Rule-based decision matrix |
...
|---|---|---|
| Phase 1 — Cross-layer join + gap report | Joins available data, shows what can and cannot be said | Now — no gate on implementation |
| Phase 2 — Bottleneck diagnosis | Runs decision matrix, flags the weakest layer | ≥2 layers with ≥4 weeks of real data |
| Phase 3 — Weekly action generation | Outputs one ranked action with evidence | Phase 2 validated by founder |
| Phase 4 — Trend tracking | Tracks bottleneck layer over time, surfaces shifts | ≥8 weeks of joint data |

**The implementation target for V10 Phase 1 is a script that runs honestly on empty or
partial data — surfacing what it knows, what it cannot say yet, and what the founder
...
|---|---|
| Make decisions autonomously | V10 outputs recommendations — the founder decides |
| Run bottleneck diagnosis from a single week | One week is not a pattern (gate G5) |
| Recommend paid advertising | No CAC/LTV baseline — cannot justify spend |
| Automate any product, distribution, or business action | V10 is read-only across all three layers |
| Recommend product changes | V7 handles product investigation — V10 only surfaces known V7 signals |
| Declare a bottleneck layer without evidence from that layer | Missing data = "cannot assess" — not a guess |
| Replace the V7, V8, or V9 layer-specific reports | V10 supplements; it does not replace |
| Claim product-market fit, virality, or growth trajectory | Anti-hype — same discipline as V8 and V9 |
...
|---|---|---|---|
| `data/benchmark_observations.jsonl` | V7 — Product | `date`, `metric`, `value`, `unit`, `scenario` | No `period` field — ISO week derived from `date` |
| `data/business_observations.jsonl` | V8 — Business | `period`, `metric`, `value`, `status`, `source` | `period` = ISO week `YYYY-Www` directly |
| `data/content_observations.jsonl` | V9 — Distribution | `period`, `platform`, `content_type`, `views`, `record_type`, `source` | `period` = ISO week `YYYY-Www` directly |

### 3a — V7 period derivation
...
appears in the unified table. Weeks with no data from a layer show `—` for that
layer's columns.

### 3c — Validation sample filter (applied before join)
...
- V7: no validation source concept — all records are treated as real
...
The script is safe to run repeatedly — each run reflects the latest data state.

---

## 5. Decision Report Format

### 5a — `unified_weekly_report.md` sections
...
|---|---|---|---|---|
| V7 Product | N | W weeks | paste_execute=645ms | ✅/⚠️/❌ |
| V8 Business | N | W weeks | downloads=?, MRR=? | ✅/⚠️/❌ |
| V9 Distribution | N | W weeks | posts=?, views=? | ✅/⚠️/❌ |
...
<Only shown if ≥2 layers have ≥4 weeks of real data>
OR
> ⚠️ Insufficient data for diagnosis. [N] layer(s) have fewer than 4 weeks of real observations.
...
**Why:** <Evidence from the data — cite specific metrics>
...
V10 Phase 1 uses a rule-based decision matrix — no machine learning, no scoring formula.
The three possible bottleneck states are assessed in priority order.

### 6a — Bottleneck priority order

The operating contract defines product improvement as highest priority.
V10 respects this by checking layers in order:

```
1. Is the product constraint unresolved?  → if YES → bottleneck = Product
2. Is the funnel constraint unresolved?   → if YES → bottleneck = Funnel
3. Is the distribution constraint present? → if YES → bottleneck = Distribution
4. Is data insufficient to assess?        → if YES → output = Data Entry
```

### 6b — Product constraint signal (V7)
...
### 6c — Funnel constraint signal (V8)

The funnel constraint is assessed when V8 has ≥1 real week of data:
...
### 6d — Distribution constraint signal (V9)

The distribution constraint is assessed when V9 has ≥1 real week of data:
...
1. V8 business (most actionable — Monday session takes 10 minutes)
...
|---|---|---|---|
| ✅ Yes | Any | Any | Fix product — paste_execute or RAM issue unresolved |
| ❌ No | ✅ Yes | Any | Fix funnel — improve activation flow or onboarding |
| ❌ No | ❌ No | ✅ Yes | Scale distribution — post more, test new platforms |
| ❌ No | ❌ No | ❌ No | 🎉 All layers healthy — maintain and monitor |
| Insufficient data | Insufficient data | Insufficient data | Record data — no recommendation possible yet |
...
### 7a — All three layers empty (current state)

Output:
- Layer status table: all three rows show "0 observations — no real data"
- Cross-layer join table: empty
- Bottleneck diagnosis: "⚠️ No data available — cannot diagnose"
- Weekly action: "Record data — complete Monday V8 session and record first V9 post"
- Data gaps: full checklist for all three layers

### 7b — One layer has data

Output:
- Layer status: show the layer with data, mark others as empty
- Join table: single-layer rows (other columns = —)
- Bottleneck diagnosis: "⚠️ Only 1 of 3 layers has data — cross-layer diagnosis not yet possible"
- Weekly action: based on the single layer's signal

### 7c — Two layers have data
...
### 7d — All three layers have data but fewer than 4 weeks each
...
- Anti-hype: "Diagnosis from N weeks — pattern not yet confirmed"

### 7e — All three layers have ≥4 weeks of real data

Full diagnosis. MEDIUM or HIGH confidence based on signal consistency.

### 7f — Old V7 data (> 4 weeks since last benchmark)
...
- Weekly action note: "V7 data is X weeks old — run benchmarks to verify current product state"
...
### 8a — Confidence levels are explicit

Every weekly action has an explicit confidence level:
- `HIGH`: ≥4 weeks of consistent signal from ≥2 layers pointing the same direction
- `MEDIUM`: ≥2 weeks of signal from ≥2 layers, or 1 strong V7 finding
- `LOW`: 1–3 weeks of data, or only 1 layer with data
- `INSUFFICIENT DATA`: any layer with 0 real observations

### 8b — Single-week signals trigger LOW confidence

A bottleneck identified from exactly 1 week of data is always LOW confidence.
The report states: "This is a 1-week signal — confirm over 2+ consecutive weeks."

### 8c — Conflicting signals surface explicitly
...
### 8d — No derived metrics from zero denominators
...
activation_rate is not computed. The report shows "—" not "0%" or "undefined."

### 8e — No cross-layer attribution without explicit V8-V9 join
...
### 8f — Validation sample exclusion propagated to join
...
[ ] Commit weekly data:
    → git add vocalype-brain/data/ vocalype-brain/outputs/
...
|---|---|---|
| G1 — Validation filter | All three layers filtered before join | Report marks the layer as empty if all records are validation-only |
| G2 — Minimum data for diagnosis | Bottleneck diagnosis requires ≥2 layers with real data | Report shows "insufficient data" section only |
| G3 — No stale-data diagnosis | V7 benchmarks >4 weeks old are flagged as STALE | Diagnosis confidence = STALE; action = re-run benchmarks |
| G4 — No derived metric from zero | activation_rate computed only when both inputs are real measured values | Report shows "—" not a computed rate |
| G5 — No single-week bottleneck | Bottleneck declared only after ≥2 consecutive weeks of same signal | Confidence = LOW on first week; MEDIUM on second; HIGH on ≥4 |
| G6 — No cross-layer attribution | Content-to-download attribution requires V9 Phase 2 script | Placeholder shown; no attribution claimed |
| G7 — No paid recommendations | Report never suggests paid advertising | Hardcoded exclusion |
| G8 — No action from empty data | Weekly action requires ≥1 layer with real observations | If all empty: action = "record data" only |
...
| # | Condition | Action |
|---|---|---|
| SC1 | All three layers have 0 real observations | Output data-entry checklist only — no diagnosis |
| SC2 | V7 data shows a new metric > 2× previous baseline | Flag anomaly — may indicate a regression, re-run benchmark before acting |
| SC3 | V8 MRR drops > 30% week-over-week | Surface immediately as anomaly — do not bury in weekly action |
| SC4 | V9 views > 500K in a single week | Flag for verification — may be viral outlier requiring attribution check |
| SC5 | Conflicting layer signals without explanation | Surface conflict explicitly — do not choose a side silently |
| SC6 | Weekly action confidence = INSUFFICIENT DATA for ≥4 consecutive weeks | Flag: "4 weeks without sufficient data — data recording has stalled" |
| SC7 | V8 shows churn > paid_conversions for 2+ consecutive weeks | Escalate immediately — product or pricing issue |
| SC8 | Script asked to automate actions, post content, or call external APIs | Stop immediately — outside V10 scope |
...
|---|---|---|
| 1 | `generate_unified_report.py` — reads all three layers, derives V7 periods, joins by week, writes both output files | `feat(brain)` |
| 2 | `outputs/unified_weekly_report.md` — generated on validation run (shows 0 real obs, data entry checklist) | `feat(brain)` |
| 3 | `outputs/weekly_action.md` — generated on validation run (shows INSUFFICIENT DATA action) | `feat(brain)` |
| 4 | Validate with existing V7 benchmark data (which DOES exist — 43 observations) | `feat(brain)` |
| 5 | Update `current_state.md` | `docs(brain)` |

### Phase 2 — Bottleneck diagnosis (after ≥2 layers have ≥4 weeks of real data)
...
| 8 | `data/unified_snapshots.jsonl` — append weekly join snapshot for trend tracking | `feat(brain)` |

### Phase 3 — Trend tracking (after ≥8 weeks of joint data)

| Step | Deliverable | Type |
|---|---|---|
| 9 | Add trend section to unified report (is the bottleneck layer shifting?) | `feat(brain)` |
| 10 | `outputs/bottleneck_trend.md` — which layer was flagged each week | `feat(brain)` |
...
```bash
# Syntax check
python -m py_compile vocalype-brain/scripts/generate_unified_report.py
...
# - V8 layer: "0 real observations — data entry pending"
# - V9 layer: "0 real observations — data entry pending"
# - Bottleneck: "Insufficient data for full diagnosis — V7 product constraint flagged"
# - Weekly action: product layer (paste_execute known issue) — MEDIUM confidence
# - No validation sample data in either output file

# Confirm validation samples not in outputs
grep -c "manual_validation" vocalype-brain/outputs/unified_weekly_report.md
grep -c "manual_validation" vocalype-brain/outputs/weekly_action.md
# Both should return 0

# Check git status
git status --short
# Should show only vocalype-brain/ files
```

**Special validation — V7 partial data run:**
...
| Anti-hype gates are hardcoded | V11 inherits them automatically — no new safety logic needed |
| Validation sample exclusion runs before join | V11 inherits it — no new filtering needed |
| `data/unified_snapshots.jsonl` (Phase 2) appends per run | V11 can track report history without reading raw output files |

**V11 gate (preliminary — V11 will define its own):**
- V10 Phase 2 diagnosis validated by founder over ≥4 weeks
...
Mission:
Implement V10 Phase 1 — Unified Decision Engine (minimal).
...
Create:
- vocalype-brain/scripts/generate_unified_report.py
- vocalype-brain/outputs/unified_weekly_report.md    (generated)
- vocalype-brain/outputs/weekly_action.md             (generated)
...
Section 6: Decision scoring model
  - Rule-based decision matrix (not ML) — Sections 6a–6f
  - Product check → Funnel check → Distribution check → Insufficient data
  - paste_execute > 300ms → product constrained
  - activation_rate < 30% (if real V8 data exists) → funnel constrained
  - 0 content posts last 2 weeks (if real V9 data exists) → distribution constrained

Section 7: Missing data fallback
  - 7a: All empty → data entry checklist only
  - 7b/7c: Partial data → partial diagnosis with missing-layer note
  - 7d: <4 weeks → LOW confidence
  - 7e: ≥4 weeks → MEDIUM/HIGH confidence
  - 7f: V7 stale (>4 weeks old) → STALE flag on product assessment
...
Section 10: Safety gates G1–G8 must be implemented
...
generate_unified_report.py requirements:
  ...
  - Runs bottleneck diagnosis (Phase 1: product constraint only — V8 and V9 may be empty)
...
- Run python -m py_compile vocalype-brain/scripts/generate_unified_report.py
...
- Run git status --short — confirm only vocalype-brain/ files changed
...
Inputs:
  data/benchmark_observations.jsonl  — V7 product (43 real obs exist)
  data/business_observations.jsonl   — V8 business (0 real obs)
  data/content_observations.jsonl    — V9 distribution (0 real obs)

Outputs:
  outputs/unified_weekly_report.md   — cross-layer join table + diagnosis
  outputs/weekly_action.md           — one ranked action with evidence

Decision matrix (rule-based, not ML):
  1. Product constrained?  → Fix product first (paste_execute, RAM)
  2. Funnel constrained?   → Fix activation before scaling distribution
  3. Distribution issue?   → Post more content, test new platforms
  4. All layers healthy?   → Maintain and monitor
  5. Insufficient data?    → Record data — no recommendation yet

Current V10 output (with today's data):
  V7: MEDIUM confidence — paste_execute=645ms product constraint known
  V8: INSUFFICIENT DATA — 0 real business observations
  V9: INSUFFICIENT DATA — 0 real content observations
...
Scripts to build (Phase 1):
  generate_unified_report.py   — single script, two outputs

V11 link:  V10 runs on-demand now → V11 runs on schedule (Monday 08:00)
           V10 proves the format; V11 automates the cadence

Phase 1:  Now (implement generate_unified_report.py)
Phase 2:  After ≥2 layers have ≥4 weeks of real data (bottleneck diagnosis)
Phase 3:  After ≥8 joint weeks (trend tracking, bottleneck shift detection)
