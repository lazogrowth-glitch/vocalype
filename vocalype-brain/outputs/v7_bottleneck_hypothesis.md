# Vocalype V7 — Bottleneck Hypothesis Report

Date: 2026-04-24
Task type: measurement_task / planning_only
Status: Hypothesis — not yet confirmed. Do not optimize until baseline is locked.
...
| `capture_duration_ms` (audio length) | 3 | 3871 ms | — | 1831 ms | 7410 ms |
...
| `chunk_cleanup` | 0 ms | 0% (median) | LLM cleanup, optional — fires 9/38 runs, up to 3320ms |
| `dictionary_replacement` | 0 ms | 0% (median) | Optional — fires 17/38 runs, up to 446ms |
| `paste_execute` | 645 ms | ~62% | **Dominant cost** |

**Startup timing (7 launches, from `[startup]` log lines):**

- `initialize_core_logic`: 30–44ms (avg 35ms) — negligible
- `model pre-warm launched`: async, completion time NOT logged
- `Microphone stream initialized`: 139–239ms (avg 197ms)
...
|---|---|---|
| Post-recording pipeline time (recording-stop → paste) | ✅ Yes | High — 38 real log samples |
| Per-chunk STT inference time | ✅ Yes | High — 63 log samples |
| `paste_execute` time | ✅ Yes | High — 38 samples, very consistent |
| Audio capture duration | ✅ Partial | Medium — WAV file sizes, 20 samples |
| Synchronous app startup time | ✅ Yes | High — 7 launches |
| Trigger-to-recording-start time | ❌ No | Not logged |
| Model pre-warm load time | ❌ No | `model pre-warm launched` logged, completion is not |
| `chunk_cleanup` LLM time (when triggered) | ✅ Partial | High when triggered — but trigger condition unknown |
| `dictionary_replacement` time (when triggered) | ✅ Partial | High when triggered |
...
|---|---|
| `app_idle_ram_mb` | Unknown memory footprint — crashes on low-RAM machines |
| `ram_during_transcription_mb` | Peak memory may spike during inference |
| `ram_after_transcription_mb` | Memory leak detection |
| `model_load_time_ms` | Cold-start UX — how long until first dictation is possible |
| `wer_percent` / `cer_percent` | Accuracy unknown — errors kill trust even if latency is good |
| `activation_success_rate` | How often does the app reach "ready" without failure? |
| `first_successful_dictation_time_ms` | Total onboarding time — key North Star activation metric |
| Trigger → recording-start gap | First ~50–200ms of perceived latency is invisible in current logs |
...
> in a median of **230ms per chunk**. The `paste_execute` step — keyboard/clipboard
> injection into the target application — takes a **constant ~645ms**, regardless
...
If inference → 0ms:                     ~740 ms  (30% improvement)
If paste → 300ms:                       ~698 ms  (33% improvement)
If paste → 300ms + inference → 150ms:  ~545ms   (48% improvement)
```

The paste step is the higher-leverage target — and it requires no model change.
...
|---|---|---|
| H1: paste-bound | **Medium-High** | paste_execute is consistently 645ms across all 38 runs — very unlikely to be noise. BUT: the cause of the 645ms is unknown (OS injection? clipboard delay? Enigo timing?). |
| H2: chunk_cleanup as tail-latency driver | **High** | 100% of p95 outliers have chunk_cleanup > 0ms. Clear pattern. |
| Inference is NOT the bottleneck | **High** | 230ms median inference vs 645ms median paste. 2.8× difference is not measurement noise. |
...
3. Trigger → recording-start gap is unmeasured — total end-to-end trigger-to-paste could be longer.
4. RAM and WER are completely unmeasured — optimizing latency while crashing on low-RAM machines or producing wrong text is not a win.
...
| WER / transcription quality | No baseline — cannot know if a change helps or hurts. |
| RAM optimizations | No RAM baseline — cannot demonstrate improvement. |
| Activation flow | `activation_success_rate` and `first_successful_dictation_time_ms` are unmeasured — the North Star metric is blind. |

> The operating contract stop condition is clear:
> **Do not begin optimization until ≥5 observations exist for every priority metric.**
...
In priority order — each requires no product code modification:

### 7A — `paste_execute` root cause (observation, not fix)
...
3. **Cross-reference with settings**: the `RuntimeDiagnostics` struct logs `paste_method` and `clipboard_handling` — check what values are reported in the log.
...
### 7B — Missing priority metrics (manual, 30 minutes)

Run the M1 session from `manual_benchmark_needed.md`:
- `app_idle_ram_mb` — Task Manager while app is idle
- `model_load_time_ms` — stopwatch from launch to first "ready"
- `ram_during_transcription_mb` — Task Manager peak during spinner
- `ram_after_transcription_mb` — Task Manager 5s after paste

### 7C — `chunk_cleanup` trigger condition
...
This is **read-only source inspection** — no code changes.

### 7D — WER baseline (manual, 20 minutes)
...
> ⚠️ This section names a potential investigation area.
> It is NOT an implementation handoff. Do NOT generate code. Do NOT create a handoff task.
> The operating contract requires: measure → diagnose → propose → implement small.
...
(bringing p50 total from ~1043ms to ~450ms — a 57% improvement in user-perceived latency).
...
| Confirmation bias | The hypothesis focuses on paste because the data is clear. RAM, WER, and activation data are missing — those might reveal a worse bottleneck. | Complete the full 10-metric baseline before prioritizing any single fix. |
| Total latency undercount | Current `total_dictation_latency_ms` measures recording-stop → paste. The trigger → recording-start gap (hotkey recognition, audio device open) is NOT included. Real end-to-end may be 50–200ms higher. | Add trigger-to-recording timing to log analysis (read-only source check). |
...
Step 5 (analysis):
  ...
  If baseline ready: run lock_benchmark_baseline.py --approve (V7 Phase 2 — not yet built).
...
Pipeline p50 breakdown:
  ...
  paste_execute               645ms   62%  ← DOMINANT
  other steps                  95ms    9%
  TOTAL                      1043ms  100%
...
NEXT ACTION:
  Read paste mechanism (Enigo, src-tauri/Cargo.toml) — read-only
