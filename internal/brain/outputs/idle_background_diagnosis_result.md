# Idle Background Inference Loop â€” Diagnosis Result
...
### RC-1 â€” Wake-word idle inference loop
...
### RC-2 â€” Stuck recording session
...
during an active recording session. Chunk idx=83â€“99 confirms the session had been
...
= 600 ms for Parakeet V3, defined in `managers/audio.rs:255â€“261`) occasionally classifies
...
(`PARAKEET_V3_MULTI_CHUNK_INTERVAL_SAMPLES = 8 Ã— 16,000 = 128,000 samples`, from
...
- Silently dropped by the binding_id mismatch guard at `transcribe.rs:1169â€“1176`
- Interrupted during wake-word auto-stop (unlikely â€” wake_word is disabled)
...
2. `processed_samples` slowly growing as VAD-classified ambient frames accumulate
   (bounded by `MAX_PROCESSED_SAMPLES = 16,000 Ã— 300 = 4.8M samples = ~19 MB` at
   `recorder.rs:424`, but drain keeps it trimmed)
3. Windows heap fragmentation from repeated ~500 KBâ€“3 MB allocation/free cycles
...
| RC-2 (stuck recording session) still relevant? | **Yes â€” primary cause** |

**Recommendation: Investigate stuck recording session (RC-2) before any wake-word patch.**

### Next action

**Task type:** `investigation_only` â†’ then `implementation_task`

**Focus:** Why does a recording session fail to stop cleanly?

**Investigation targets (read-only):**

1. `src-tauri/src/actions/transcribe.rs:1149â€“1176` â€” `stop_transcription_action`: the
...
2. `src-tauri/src/runtime/transcription_coordinator.rs` â€” `send_input` debounce at line
...
Add a maximum session duration guard in the chunking sampler thread â€” if the session has
...
- `internal/brain/outputs/idle_background_transcription_diagnosis.md` â€” written (previous step)
- `internal/brain/outputs/idle_background_diagnosis_result.md` â€” this file
- `internal/brain/data/results.jsonl` â€” new entry appended
- `internal/brain/outputs/results_report.md` â€” updated
- `internal/brain/memory/current_state.md` â€” updated
- `internal/brain/memory/lessons_learned.md` â€” new entry
