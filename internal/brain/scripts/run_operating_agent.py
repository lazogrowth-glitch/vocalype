...
Usage:
  ...
  python internal/brain/scripts/run_operating_agent.py --skip-report-gen
  python internal/brain/scripts/run_operating_agent.py --dry-run
"""
...
def _write_external_audit(called: bool, external_mode: str, dry_run: bool = False) -> None:
    ...
    if context_exists and not dry_run:
        ...
        context_summary = f"- File: `internal/brain/outputs/context_pack.md`\n- Size: {size:,} bytes, {lines:,} lines\n"
    else:
        context_summary = "- File: `internal/brain/outputs/context_pack.md` (not built or dry-run)\n"
    ...
    lines_out = [
        ...
        "- `internal/brain/outputs/v11_mission_package.md` (if present)\n",
        "- `internal/brain/outputs/paste_mechanism_diagnosis.md` (if present)\n",
        "- `internal/brain/outputs/paste_utils_diagnosis.md` (if present)\n",
        "\n",
...
def _select_next_product_bottleneck(
    ...
    # --- Priority 3: Paste latency â€” post-fix benchmarks pending ---
...
def _write_fresh_investigation_mission(bottleneck: dict) -> None:
    ...
    lines = [
        ...
        "1. `internal/brain/memory/operating_contract.md` -- safety rules and operating boundaries\n",
        "2. `internal/brain/memory/current_state.md` -- current Brain state and history\n",
        "3. `internal/brain/outputs/next_product_bottleneck.md` -- bottleneck evidence and plan\n",
    ]
    idle_obs_file = BRAIN_ROOT / "outputs" / "idle_background_transcription_observation.md"
    if idle_obs_file.exists():
        lines.append(
            "4. `internal/brain/outputs/idle_background_transcription_observation.md`"
    ...
    lines += [
        ...
        "---\n\n",
        "## Deliverable\n\n",
        f"Write a single output file: `internal/brain/{output_file}`\n\n",
        "Required sections:\n\n",
        ...
        f"- Write ONLY to `internal/brain/{output_file}`\n",
...
def _write_agent_recommendation(
    ...
    if route == "observation_wait":
        ...
        if next_bottleneck:
            ...
            if nb["route"] == "sensitive_code":
                body += [
                    ...
                    "2. Open `internal/brain/outputs/next_product_bottleneck.md` for full details\n",
                ...
                body += [
                    "**How to proceed:**\n\n",
                    "1. Open `internal/brain/outputs/next_product_bottleneck.md` for details\n",
        ...
        body += [
            ...
            "```\n",
            "python internal/brain/scripts/add_business_observation.py --metric <m> ...\n",
            "```\n\n",
        ...
        if route == "data_entry" and nb_id == "paste_latency_pending_benchmarks":
            body = [
                ...
                "**Step 1 â€” Smoke test remaining apps** (if not yet done):\n\n",
                ...
                "cargo build --release\n",
                "```\n\n",
                "**Step 2 â€” Record post-fix paste latency benchmarks** (need >= 5):\n\n",
                "```\n",
                "python internal/brain/scripts/add_benchmark_observation.py \\\n",
                ...
                "**Step 3 â€” Re-run this agent** once >= 5 post-fix observations are recorded:\n\n",
                "```\n",
                "python internal/brain/scripts/run_operating_agent.py\n",
                "```\n\n",
            ...
            body = [
                ...
                "---\n\n",
                "### Priorite 1 â€” Benchmarks post-fix paste_latency_ms (besoin >= 5)\n\n",
                ...
                "---\n\n",
                "### Priorite 2 â€” Metriques business V8 (~10 min, chaque lundi)\n\n",
                "**Stripe** (ouvrir dashboard.stripe.com) :\n",
                "```\n",
                f"python internal/brain/scripts/add_business_observation.py --metric mrr --value <eur> --unit eur --source stripe --period {current_week}\n",
                f"python internal/brain/scripts/add_business_observation.py --metric paid_conversions --value <n> --unit count --source stripe --period {current_week}\n",
                f"python internal/brain/scripts/add_business_observation.py --metric trial_starts --value <n> --unit count --source stripe --period {current_week}\n",
                f"python internal/brain/scripts/add_business_observation.py --metric churned_users --value <n> --unit count --source stripe --period {current_week}\n",
                "```\n\n",
                "**Supabase** (ouvrir app.supabase.com) :\n",
                "```\n",
                f"python internal/brain/scripts/add_business_observation.py --metric account_signups --value <n> --unit count --source supabase --period {current_week}\n",
                f"python internal/brain/scripts/add_business_observation.py --metric first_successful_dictations --value <n> --unit count --source supabase --period {current_week}\n",
                "```\n\n",
                "**Vercel** (ouvrir vercel.com/analytics) :\n",
                "```\n",
                f"python internal/brain/scripts/add_business_observation.py --metric website_visitors --value <n> --unit count --source vercel --period {current_week}\n",
                f"python internal/brain/scripts/add_business_observation.py --metric downloads --value <n> --unit count --source vercel --period {current_week}\n",
                "```\n\n",
                "---\n\n",
                "### Priorite 3 â€” Contenu V9 (apres chaque publication)\n\n",
                "```\n",
                f"python internal/brain/scripts/add_content_observation.py --platform tiktok --content_type demo --period {current_week} ...\n",
                "```\n\n",
                "---\n\n",
        ...
        if external_mode == EXTERNAL_MODE_OFF:
            body = [
                ...
                "  1. Open `internal/brain/outputs/context_pack.md`\n",
                ...
                "  Then re-run: `python internal/brain/scripts/run_operating_agent.py`\n\n",
            ...
            if deepseek_called and deepseek_response_text:
                body = [
                    ...
                    "**Review the response:**\n",
                    "  Open `internal/brain/outputs/deepseek_response.md`\n\n",
                    "**Important:**\n",
                    ...
                    "**Files sent:**\n",
                    "  See `internal/brain/outputs/external_context_audit.md` for full audit.\n",
                ]
            elif deepseek_error:
                body = [
        ...
        body = [
            ...
            "**Steps:**\n\n",
            "  1. Open `internal/brain/outputs/v11_mission_package.md`\n",
        ...
        if next_bottleneck:
            ...
            body += [
                ...
                "**Steps:**\n\n",
                "1. Read `internal/brain/outputs/next_product_bottleneck.md` -- full evidence\n",
                "2. Read `internal/brain/outputs/fresh_investigation_mission.md` -- the mission\n",
...
def main() -> None:
    ...
    _p("")
    _p("  Open:")
    _p("    internal/brain/outputs/agent_recommendation.md")
    _p("    internal/brain/outputs/agent_run_report.md")
    if context_built:
        _p("    internal/brain/outputs/external_context_audit.md")
    if deepseek_called:
        _p("    internal/brain/outputs/deepseek_response.md")
    if route in ("sensitive_code", "product_implementation") and not v11_blocked_detail:
        if MISSION_PACKAGE_PATH.exists():
            _p("    internal/brain/outputs/v11_mission_package.md")
        if FRESH_MISSION_PATH.exists():
            _p("    internal/brain/outputs/fresh_investigation_mission.md  (send to Claude manually)")
    if next_bottleneck:
        _p(f"    internal/brain/outputs/next_product_bottleneck.md  ({next_bottleneck['bottleneck_id']})")
    if v11_blocked_detail:
