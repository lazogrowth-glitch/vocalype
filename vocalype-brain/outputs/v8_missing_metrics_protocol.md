# Vocalype Brain — V8C Missing / Zero Metrics Protocol

Date: 2026-04-25
Task type: planning_only
Author: Vocalype Brain
Status: design document — no scripts modified
...
The result is zero — not missing, not unknown. A confirmed absence of activity.

- Example: Stripe dashboard opened. Zero new paid conversions this week. → `zero`
- Example: TikTok analytics checked. Zero views this week. → `zero`
...
The value could be 0, 5, or 100 — we don't know.

- Example: Vercel analytics was not checked this week. → `unknown`
- Example: Supabase was not queried. → `unknown`
...
The review script will flag metrics that have been `unknown` for ≥2 consecutive weeks.
...
- Example: No analytics tool installed → `website_visitors` is `not_available`
- Example: Stripe not yet connected → `mrr` is `not_available`
- Example: No download tracking in place → `downloads` is `not_available`

Once the data source is created, the status transitions to `zero` or a real value.
`not_available` is never permanent — it is a blocker that should be resolved.
...
- Example: `trial_to_paid_rate` requires trial users to exist → `not_applicable` until first trial
- Example: `churned_users` requires paying subscribers → `not_applicable` until first conversion
- Example: `d7_retention_rate` requires ≥7 days of user history → `not_applicable` until then

`not_applicable` transitions to `zero` or a real value when the precondition is met.
It is NOT the same as `unknown` — we are not failing to check, the metric just doesn't apply.
...
|---|---|---|
| Dashboard opened, value confirmed as 0 | `zero` | Yes — `--value 0` |
| Dashboard not opened this week | `unknown` | Yes — text note only |
| Analytics tool not installed | `not_available` | Yes — text note only |
| No paying users yet; churn impossible | `not_applicable` | Yes — text note only |
| Founder guesses "probably 0" | **DO NOT USE** | No — do not guess |
| Founder copies last week's value without checking | **DO NOT USE** | No — staleness is worse than unknown |
...
## 3. Example Records — Early Vocalype Stage
...
### Week 2026-W18 — example batch

**Stripe not yet set up (or MRR is genuinely €0):**
```bash
# If Stripe exists and was checked — confirmed zero:
...
# → Do not record. Note in weekly log: "mrr: unknown this week — not checked."

# If Stripe not yet connected:
# → Note in weekly log: "mrr: not_available — Stripe not configured."
```

**Downloads — verified zero:**
```bash
python vocalype-brain/scripts/add_business_observation.py \
...
**first_successful_dictations — Supabase checked, 2 users:**
```bash
python vocalype-brain/scripts/add_business_observation.py \
...
**content_posts — founder did not post this week:**
```bash
python vocalype-brain/scripts/add_business_observation.py \
...
**churned_users — not_applicable (no paying users exist yet):**
```
# Do not record as 0. Note in weekly log:
# "churned_users: not_applicable — no paying subscribers yet."
# Record it once the first subscriber exists.
```

**website_visitors — analytics not installed:**
```
# Note in weekly log:
# "website_visitors: not_available — no analytics tool installed."
...
> The script does NOT yet support status fields — this section guides V8C script changes.

| Status | Report behaviour |
|---|---|
| Numeric value (including 0) | Display in funnel table, trend chart, weekly summary |
| `unknown` (text note) | Flag: "⚠️ metric not checked this week" — does NOT count toward baseline weeks |
| `not_available` | Flag: "🔴 data source missing — see setup checklist" — excluded from trend |
| `not_applicable` | Flag: "⏸ not applicable yet — precondition unmet" — excluded from trend |
...
**Anomaly flags:**
- `unknown` for ≥2 consecutive weeks → flag: "Metric unchecked for 2+ weeks — add to weekly routine."
- `not_available` for ≥4 weeks → flag: "Data source missing for 4+ weeks — blocking baseline."
- `not_applicable` transitions to real value → flag: "First observation — baseline clock starts."
...
2. Open Supabase (2 min)
   ...
   If you did not check: note "unknown" — do not copy last week.
...
5. Count manually (1 min)
   ...
   This is always checkable — count from your sent folder / DM history.
...
but mark them as `unknown` — do not leave them unrecorded.
...
- Funnel table: show status icons (✅ / ⚠️ / 🔴 / ⏸) next to missing metrics
...
- vocalype-brain/outputs/v8_missing_metrics_protocol.md

Mission:
Implement V8C — add --status flag to add_business_observation.py
...
Rules:
- Only modify vocalype-brain/scripts/
...
| # | Condition | Action |
|---|---|---|
| SC1 | Any numeric value was recorded without opening the data source | Remove the record — it is fabricated |
| SC2 | `not_applicable` is used to avoid checking a metric that could be checked | Change to `unknown` — `not_applicable` is structural, not optional |
| SC3 | The baseline shows ≥4 weeks with all zeros across all metrics | Flag: possible measurement setup failure — verify data sources are working |
| SC4 | `first_successful_dictations` is `unknown` for ≥2 consecutive weeks | Stop — this is the North Star metric; it must be checked every week |
| SC5 | A metric is marked `not_available` for >8 consecutive weeks | Flag as backlog blocker — the missing data source should be set up |
| SC6 | Founder asks Brain to estimate or interpolate a missing value | Refuse — record `unknown` instead, never estimate |
...
────────────────────────────────────────────────────────────────────
Four statuses:
  zero           = checked, confirmed 0    → record --value 0
  unknown        = not checked             → note only, no numeric record
  not_available  = data source missing     → note only, flag setup backlog
  not_applicable = precondition unmet      → note only, hidden from warnings
