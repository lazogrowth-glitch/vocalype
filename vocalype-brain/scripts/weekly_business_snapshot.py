"""weekly_business_snapshot.py — V8 Weekly Business Snapshot Generator.
...
def _build_snapshot(
    ...
    if checked or zeroed:
        ...
        for m, notes in zeroed:
            ...
            lines.append(f"| {desc} | ✅ 0 (confirmed) | {notes_str} |")
    else:
    ...
    if unknown:
        lines += [
            ...
            "",
            "These were not checked — status is `unknown`. They do NOT count toward the baseline.",
            "",
        ]
        for m in unknown:
            desc = METRIC_DESCRIPTIONS.get(m, m)
            source = METRIC_SOURCES.get(m, "manual")
            lines.append(f"- **{desc}** — check: {source}")
        lines.append("")
    ...
    if not_available:
        ...
        for m in not_available:
            desc = METRIC_DESCRIPTIONS.get(m, m)
            source = METRIC_SOURCES.get(m, "manual")
            lines.append(f"- **{desc}** — needs: {source}")
        lines.append("")
    ...
    if unrecorded:
        ...
        for m in unrecorded:
            desc = METRIC_DESCRIPTIONS.get(m, m)
            source = METRIC_SOURCES.get(m, "manual")
            lines.append(f"- **{desc}** — source: {source}")
        lines.append("")
    ...
    if north_star_status is None or north_star_status in ("unknown", "not_available"):
        checklist_items.append(
            "[ ] **Check North Star:** open Supabase → history table → "
            "count distinct users with first dictation this week → record `first_successful_dictations`"
        )
    ...
    for m in unknown:
        desc = METRIC_DESCRIPTIONS.get(m, m)
        source = METRIC_SOURCES.get(m, "manual")
        checklist_items.append(f"[ ] Check {source} → record `{m}`")
    ...
    checklist_items.append(
        "[ ] Commit: `git add vocalype-brain/data/business_observations.jsonl "
        "vocalype-brain/outputs/ && git commit -m \"data(brain): weekly business snapshot YYYY-Www\"`"
    )
    ...
    if not premature:
        premature.append(
            "- No specific overreaction risk flagged — but remember: one data point is not a trend."
        )
    ...
    if "first_successful_dictations" in unrecorded or north_star_status in ("unknown", None):
        next_to_record.append("`first_successful_dictations` — North Star, highest priority")
    for m in unrecorded:
        if m != "first_successful_dictations":
            source = METRIC_SOURCES.get(m, "manual")
            next_to_record.append(f"`{m}` — {source}")
    for m in unknown:
        source = METRIC_SOURCES.get(m, "manual")
        next_to_record.append(f"`{m}` — was unknown last check ({source})")
    ...
    if next_to_record:
        ...
        if len(next_to_record) > 6:
            lines.append(f"   *(and {len(next_to_record) - 6} more — see business_report.md)*")
    else:
    ...
    lines += [
        ...
        f"*Source: `vocalype-brain/data/business_observations.jsonl`*",
        f"*To record an observation: `python vocalype-brain/scripts/add_business_observation.py --help`*",
    ]
...
def main() -> None:
    ...
    print(f"\nWritten: vocalype-brain/outputs/weekly_business_snapshot.md")
    print(divider)
