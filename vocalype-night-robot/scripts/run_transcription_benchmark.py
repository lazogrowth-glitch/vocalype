...
Usage (called by robot_config.json commands.transcription_benchmark):
  python vocalype-night-robot/scripts/run_transcription_benchmark.py
"""
...
def compare(current: dict, baseline: dict) -> tuple[bool, str]:
    ...
    for metric in _LOWER_IS_BETTER:
        ...
        arrow = "↓ better" if cur < prev else ("↑ worse" if cur > prev else "= same")
        lines.append(
            f"  {metric:<42} {prev:.6f} → {cur:.6f}  {delta_pct:+.2f}%  {arrow}"
        )
...
def main() -> None:
    ...
    if baseline_entry is None:
        print("\nNo baseline found — saving current scores as baseline.")
        save_baseline(current)
        print(f"Baseline saved to: {_BASELINE_FILE}")
        print("\nRESULT: PASS (first run — baseline established)")
        sys.exit(0)
    ...
    if passed:
        ...
        if cur_primary < prev_primary:
            save_baseline(current)
            print(f"\nBaseline updated (WER improved: {prev_primary:.6f} → {cur_primary:.6f})")
