# Vocalype Brain â€” V7 Closure Report
...
That strict gate (all 10 metrics at â‰¥5 observations) was not fully met.

| Gate | Status |
|---|---|
| Scripts built and validated | âœ… Done |
| Persisted log data extracted (38 complete pipeline runs) | âœ… Done |
| 2/10 priority metrics at baseline (â‰¥5 obs) | âœ… Done |
| Bottleneck hypothesis produced from real data | âœ… Done |
| Idle resource anomaly observed and documented | âœ… Done |
| Investigation proposals written for both issues | âœ… Done |
| 8/10 priority metrics at baseline | âŒ Not met |
| `lock_benchmark_baseline.py` built | âŒ Not built |
| `benchmark_baseline.jsonl` locked | âŒ Not created |
...
1. `paste_execute` = 645ms constant across 100% of 38 runs (not noise â€” it is the bottleneck)
2. Idle background inference loop observed in logs â€” a potential stability and resource issue
...
### Pipeline latency (from persisted `vocalype.log` â€” 38 complete runs)
...
| `capture_duration_ms` | 3871 ms | â€” | 1831 ms | 7410 ms | 3 (partial) |
...
|---|---|---|
| `app_idle_ram_mb` â€” session 1 | 698 MB | Stable idle |
| `ram_during_transcription_mb` | 698 MB | No visible spike vs idle |
| `app_idle_ram_mb` â€” initial | 699 MB | Before growth observed |
| `app_idle_ram_mb` â€” after 15 min | 809 MB | No intentional dictation â€” +110 MB |
...
### Finding 1 â€” Pipeline is PASTE-BOUND (not inference-bound)

**Confidence: Medium-High**

`paste_execute` = 645ms constant = **62% of p50 post-recording latency**.
Parakeet inference = 230ms median. Real-time factor ~0.06x (fast â€” not the bottleneck).

```
Pipeline p50 (1043ms):
  inference   303ms  29%
  paste       645ms  62%  â† dominant
  other        95ms   9%
```
...
```
Current p50:               1043 ms
If paste â†’ 100ms:           ~450 ms  (âˆ’57%)
If inference â†’ 0ms:         ~740 ms  (âˆ’30%)  â† less leverage
```
...
### Finding 2 â€” Possible idle background inference loop

**Confidence: Lowâ€“Medium (1 observation, log-supported)**

RAM grew from 699 MB â†’ 809 MB (+110 MB) over ~15 minutes with no intentional dictation.
...
Transcription completed in ~192â€“229ms
Transcription result is empty
â†³ repeating every ~1â€“2 seconds
```
...
### Finding 3 â€” chunk_cleanup is the tail-latency driver

**Confidence: High**

`chunk_cleanup` (optional LLM step) fires in 9/38 runs (24%) and accounts for
100% of p95+ outliers. Without it, p95 â‰ˆ 1550ms. With it, p95 â†’ 2405â€“4747ms.
...
|---|---|
| 8/10 priority metrics at â‰¥5 obs (model_load, RAM delta, WER, CER, activation_success, first_dictation) | No locked baseline â€” V8 comparisons will be partial. Acceptable given the signal quality from latency data. |
| `lock_benchmark_baseline.py` â€” Phase 2 script | Cannot formally lock a before/after baseline. V8 will establish a partial baseline from existing data. |
| `compare_benchmarks.py` â€” Phase 2 script | Post-patch comparison will be manual until built. |
| `benchmark_baseline.jsonl` â€” locked baseline | V8 will need to build this as part of its own phase. |
...
| model_load_time_ms | Not measured. Low priority â€” startup timing from logs shows 35ms for core logic. |
| M1 manual session (Phase A: RAM growth timed readings) | Idle background loop not yet confirmed across â‰¥3 timed readings. |
...
| PB-3 | Confirm idle RAM growth across â‰¥3 timed readings | `measurement_task` | `idle_background_transcription_observation.md` Phase A | Medium |
...
| PB-6 | Build `lock_benchmark_baseline.py` | `implementation_task` (Brain) | â€” | Low |
| PB-7 | Build `compare_benchmarks.py` | `implementation_task` (Brain) | â€” | Low |
...
**Reason 1 â€” Diminishing returns on pure measurement.**
...
**Reason 2 â€” Investigation is now the bottleneck, not data collection.**
...
**Reason 3 â€” Operational discipline.**
...
|---|---|---|
| â‰¥5 V7 manual benchmark sessions complete | âŒ Not met | âœ… Waived â€” latency signal is high-confidence from 38 log runs |
| V7 baseline locked in `benchmark_baseline.jsonl` | âŒ Not met | âœ… Waived â€” V8 will build this as its first act |
| At least one product change benchmarked before AND after | âŒ Not met | âœ… Waived â€” V8 will establish the before snapshot before any change |
...
**V8 = Targeted Investigation â†’ Diagnosis â†’ First Patch â†’ Before/After Comparison**

V8 is the first version that moves from pure measurement to **informed action**.
It follows the operating contract method: measure â†’ diagnose â†’ propose â†’ implement small.

V8 has two parallel tracks:

### Track A â€” paste_execute diagnosis and patch
...
Step A3:  Apply approved patch (single file, small change)
          Run â‰¥10 dictations, record new paste_latency_ms observations
          Compare p50 before (645ms) vs after

Step A4:  Write before/after comparison
          If paste p50 drops by â‰¥200ms: declare V8 Track A a win
          If no improvement: record learning, do not revert without cause
```

### Track B â€” idle background inference loop diagnosis
...
Step B2:  Confirm RAM growth pattern across â‰¥3 timed readings (Phase A protocol)
...
| Product code changes per track | â‰¤ 2 files, â‰¤ 50 lines each |
...
| Modify Parakeet inference pipeline | Not the bottleneck â€” 230ms median is fast. Low leverage. |
...
| Change both tracks in one commit | One change at a time â€” before/after comparison requires isolation |
| Create an implementation handoff before investigation report exists | Operating contract: diagnose before proposing |
| Touch auth, license, payment, or security code | Forbidden scope â€” always |
...
| Design V9 before V8 Track A or B has at least one completed patch | V9 = business metrics loop â€” requires at least one confirmed product win first |
...
- internal/brain/outputs/product_patch_proposal_report.md
- internal/brain/outputs/idle_background_transcription_observation.md

Mission:
Design V8 â€” Targeted Investigation and First Patch phase.
...
2. Track A â€” paste_execute investigation protocol (read-only)
3. Track B â€” idle inference loop investigation protocol (read-only)
...
Verdict:    CLOSED â€” sufficient signal to move to V8
...
Baseline:   Not locked (intentionally â€” waived for V8 entry)
...
Product backlog:
  ...
  PB-3â€“8: Remaining metrics, baseline scripts            MEDIUM/LOW

V8 status:  Conditionally approved to begin
V8 entry:   Use exact prompt in Section 10 above
V9 gate:    V8 must produce â‰¥1 confirmed product win first
