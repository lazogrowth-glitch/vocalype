"""generate_unified_report.py — V10 Phase 1 Unified Decision Engine.
...
Decision matrix (rule-based, not ML — evaluated in priority order):
  1. Product constrained?  → product_investigation (fix before scaling)
  2. Funnel constrained?   → business_data_entry   (fix activation first)
  3. Distribution issue?   → distribution_data_entry (post more content)
  4. Insufficient data?    → business_data_entry   (record data first)
  5. All healthy?          → hold                  (maintain and monitor)
...
def _diagnose(
    ...
    if not v7_has and not v8_has and not v9_has:
        return dict(
            ...
            next_step=(
                ...
                "using: python vocalype-brain/scripts/add_business_observation.py"
            ),
            what_not_to_do=(
    ...
    # ── Priority 1: product constraint (V7 data exists and signals a problem) ─
    if v7_has and v7["product_constrained"] and not v7["stale"]:
        ...
        # MEDIUM: V7 has data but V8/V9 are empty — can't cross-validate business impact
        ...
        return dict(
            ...
            action=(
                "Investigate `paste_execute` root cause — "
                "read-only inspection of src-tauri/src/actions/paste.rs"
            ),
            evidence=evidence,
            why_not_other=(
                "V7 benchmarks confirm paste_execute ≈644ms = ~62% of p50 dictation latency. "
                "Scaling distribution or improving the funnel before fixing this sends users "
                "into a sluggish product experience. "
                "V8/V9 have no real data yet — but fixing product before measuring growth "
            ...
            next_step=(
                ...
                "No code changes during this investigation — diagnosis only."
            ),
            what_not_to_do=(
                "Do not implement any code change before the diagnosis is written. "
                "Do not optimise Parakeet inference (stt_inference_time=~230ms — not the bottleneck). "
    ...
    if v7_has and v7["stale"]:
        return dict(
            ...
            confidence="low",
            action_type="product_investigation",
            action=f"Re-run V7 benchmarks — last observation is {v7['data_age_weeks']} weeks old",
            ...
            next_step=(
                "Run ≥5 manual dictation sessions and record each with: "
    ...
    if v8_has and v8["funnel_constrained"] is True:
        ...
        return dict(
            ...
            action_type="business_data_entry",
            action=f"Improve activation flow — activation rate is {rate_str} (target ≥30%)",
            evidence=[
            ...
            why_not_other=(
                ...
                "more people into the same broken funnel — wasted effort."
            ),
            next_step=(
    ...
    if v9_has and v9["distribution_constrained"] is True:
        return dict(
            ...
            action="Resume content posting — no posts recorded this week",
            evidence=[
    ...
    # ── Insufficient cross-layer data ───────────────────────────────────────
    if not v8_has and not v9_has:
        return dict(
            bottleneck="insufficient_data",
            confidence="insufficient_data",
            action_type="business_data_entry",
            action="Record real business metrics — V8 has no real observations",
            evidence=[
    ...
    return dict(
        ...
        confidence="medium",
        action_type="hold",
        action="All three layers are healthy — maintain current direction",
        evidence=[],
...
def _build_unified_report(
    ...
    def suf_icon(has_data: bool, weeks: list) -> str:
        if not has_data:
            return "❌ No real data"
        return "✅ Sufficient" if len(weeks) >= 4 else f"⚠️ {len(weeks)} week(s) — needs ≥4"
    ...
    if data_state == "EMPTY":
        lines += [
            "> ⚠️  No real observations found in any layer.",
    ...
    stale_note = f" ⚠️ {v7['data_age_weeks']}w old" if v7["stale"] else ""
    ...
    lines += [
        ...
        "|---|---|---|---|---|",
        f"| **V7 — Product** | {v7['total_obs']} | {len(v7['weeks'])} | {v7_signal}{stale_note} | {suf_icon(v7['total_obs'] > 0, v7['weeks'])} |",
        f"| **V8 — Business** | {v8['total_real_obs']} ({v8['validation_count']} excluded) | {len(v8['weeks'])} | {v8_signal} | {suf_icon(v8['total_real_obs'] > 0, v8['weeks'])} |",
        f"| **V9 — Distribution** | {v9['total_real_obs']} ({v9['validation_count']} excluded) | {len(v9['weeks'])} | {v9_signal} | {suf_icon(v9['total_real_obs'] > 0, v9['weeks'])} |",
        "",
    ]
    lines += ["---", ""]
    ...
    if join_rows:
        ...
        lines += ["> No data in any layer yet — join table is empty.", ""]
    lines += ["---", ""]
    ...
    if v7["total_obs"] > 0:
        if v7["constraint_reasons"]:
            ...
            for reason in v7["constraint_reasons"]:
                lines.append(f"| Product constraint | {reason} | ⚠️ Unresolved |")
            if v7["idle_loop_confirmed"]:
                lines.append(f"| Stability risk | idle_background_inference_loop confirmed in logs | ⚠️ Unresolved |")
            lines.append("")
        ...
        if v7["stale"]:
            lines += [
                f"> ⚠️ V7 data is **{v7['data_age_weeks']} weeks old** — may not reflect current product state.",
    ...
    if diagnosis["bottleneck"] == "insufficient_data":
        lines += [
            "> ⚠️ Insufficient data for full cross-layer diagnosis.",
            ">",
            "> **What IS known:**",
        ]
    ...
    if v9["total_real_obs"] == 0:
        gaps.append(
            ...
            "After publishing, record each post with: `python vocalype-brain/scripts/add_content_observation.py`"
        )
    elif len(v9["weeks"]) < 4:
        gaps.append(
    ...
    if v7["stale"]:
        gaps.append(
            ...
            "Run ≥5 manual benchmark sessions using `add_benchmark_observation.py`."
        )
    ...
    if v7["product_constrained"] and not v7["stale"]:
        risks.append("**MEDIUM — Product latency:** paste_execute ≈644ms unresolved. Users experience visible paste lag on every dictation.")
    if v7["idle_loop_confirmed"]:
        risks.append("**MEDIUM — Memory leak:** idle background inference loop confirmed. RAM grew +110MB in 15 min idle.")
    if v8["total_real_obs"] == 0:
        risks.append("**LOW — Business blind spot:** 0 real V8 observations. Cannot detect funnel failures or MRR changes.")
    if v9["total_real_obs"] == 0:
        risks.append("**LOW — Distribution blind spot:** 0 real V9 observations. Cannot assess whether content drives downloads.")
    if not risks:
    ...
    lines += [
        "| # | Condition | Status |",
        "|---|---|---|",
        f"| SC1 | All three layers have 0 real observations | {'🔴 Active' if layers_with_data == 0 else '✅ Clear'} |",
        f"| SC3 | V8 MRR drops > 30% week-over-week | {'N/A — no V8 data' if v8['total_real_obs'] == 0 else '✅ Monitoring'} |",
        f"| SC6 | 4+ consecutive weeks without sufficient data | {'⚠️ Not yet — check in 4 weeks' if layers_with_data < 3 else '✅ Clear'} |",
        "",
    ]
    lines += ["---", ""]
    ...
    if v8["total_real_obs"] == 0:
        lines.append("[ ] **Record V8 business metrics** (10-min session — Stripe, Supabase, Vercel)")
    if v9["total_real_obs"] == 0:
    ...
    lines += [
        ...
        "[ ] Commit: `git add vocalype-brain/data/ vocalype-brain/outputs/ && "
    ...
    if data_state == "PARTIAL":
        lines += [
            f"- {layers_with_data}/3 layers have real data — full cross-layer pattern requires all three.",
        ...
        lines += ["- All layers have data — check confidence level before acting on any single week's signal."]
    ...
    lines += [
        ...
        f"*To re-run: `python vocalype-brain/scripts/generate_unified_report.py`*",
    ]
...
def _build_weekly_action(
    ...
    if diagnosis["evidence"]:
        ...
        lines += ["> No layer-specific evidence available — diagnosis based on absence of data.", ""]

    # Also show layer data state
    lines += [
        "| Layer | Real observations | Status |",
        "|---|---|---|",
        f"| V7 — Product | {v7['total_obs']} | {'⚠️ Constraint confirmed' if v7['product_constrained'] else ('✅ No constraint' if v7['total_obs'] > 0 else '❌ No data')} |",
        f"| V8 — Business | {v8['total_real_obs']} | {'⚠️ Funnel issue' if v8['funnel_constrained'] else ('✅ Healthy' if v8['total_real_obs'] > 0 else '❌ No data')} |",
        f"| V9 — Distribution | {v9['total_real_obs']} | {'⚠️ Gap detected' if v9['distribution_constrained'] else ('✅ Posting' if v9['total_real_obs'] > 0 else '❌ No data')} |",
        "",
    ]
    lines += ["---", ""]
    ...
    if diagnosis["confidence"] == "medium":
        lines += [
            f"**{conf_str}** — Signal is real but cross-layer validation is incomplete.",
            "",
            "What would raise confidence to HIGH:",
        ]
        if v8["total_real_obs"] == 0:
            lines.append("- Record ≥4 weeks of V8 business data (enable funnel cross-validation)")
        if v9["total_real_obs"] == 0:
            lines.append("- Record ≥4 weeks of V9 content data (enable distribution cross-validation)")
        if v7["stale"]:
            lines.append("- Re-run V7 benchmarks (data is stale)")
    elif diagnosis["confidence"] == "insufficient_data":
        lines += [
            f"**{conf_str}** — Cannot make a confident recommendation without real observations.",
            "",
        ...
        lines += [f"**{conf_str}** — All three layers have ≥4 weeks of consistent data pointing in the same direction."]
    elif diagnosis["confidence"] == "low":
        lines += [
            f"**{conf_str}** — Signal is present but from limited data (< 2 weeks or 1 layer only).",
    ...
    lines += [
        ...
        f"*To update: record new data then re-run `python vocalype-brain/scripts/generate_unified_report.py`*",
    ]
...
def main() -> None:
    ...
    print(f"\nWritten: vocalype-brain/outputs/unified_weekly_report.md")
    print(f"Written: vocalype-brain/outputs/weekly_action.md")
    print(divider)
