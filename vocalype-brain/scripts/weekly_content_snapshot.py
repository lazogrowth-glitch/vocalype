"""weekly_content_snapshot.py — V9 Weekly Content Snapshot Generator.
...
The snapshot is:
  ...
  - Anti-hype: "N weeks of data — not a trend yet" until ≥4 weeks
...
def _build_snapshot(observations: list[dict[str, Any]], now: datetime) -> str:
    ...
    lines += [
        "# Vocalype Brain — Weekly Content Snapshot",
        "",
    ...
    if not has_real_data:
        lines += [
            "> ⚠️  No real content observations found — only validation samples exist.",
    ...
    if week_post_list:
        ...
        for p in week_post_list:
            ...
            if p.get("hook") and len(p["hook"]) > 45:
                hook += "..."
            views = f"{p.get('views', '—'):,}" if isinstance(p.get("views"), int) else "—"
            status = "✅ has data" if _has_performance(p) else "⏳ pending check"
    ...
    if pending:
        ...
        lines += ["These posts have no performance data yet — check platform analytics 24–72h after posting:", ""]
        for p in pending:
            post_id = p.get("post_id", "—")
            platform = p.get("platform", "—")
    ...
    if pending:
        checklist.append(
            f"[ ] Record performance data for {len(pending)} pending post(s) — see above"
        )

    # Always: run review
    checklist.append(
        "[ ] Run: `python vocalype-brain/scripts/review_content_performance.py`"
    )
    checklist.append(
        "[ ] Commit: `git add vocalype-brain/data/content_observations.jsonl "
        "vocalype-brain/outputs/ && git commit -m \"data(brain): weekly content snapshot YYYY-Www\"`"
    )
    ...
    if checklist:
        ...
        lines.append("> No checklist items — add performance data and next_action fields to generate actions.")
    lines.append("")
    ...
    if weeks_of_data < MIN_WEEKS_FOR_TREND:
        premature.append(
            f"- {weeks_of_data} week(s) of data — baseline requires ≥{MIN_WEEKS_FOR_TREND} weeks "
    ...
    if posts_with_views and max(p.get("views", 0) for p in posts_with_views) < 100:
        premature.append(
            "- Low views on early posts is normal — reach grows as the account grows."
        )
    if not premature:
        premature.append(
            "- No specific overreaction risk flagged — but remember: one week is not a trend."
        )

    lines += premature
    lines += [
        "",
        "> Trend analysis requires ≥4 weeks of consistent posting across ≥2 platforms.",
        "",
    ]
...
def main() -> None:
    ...
    print(f"\nWritten: vocalype-brain/outputs/weekly_content_snapshot.md")
    print(divider)
