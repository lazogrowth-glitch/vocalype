"""
model_route_decision.py â€” Vocalype Brain Operating Mode
========================================================
...
Outputs a recommendation to the terminal and to:
  internal/brain/outputs/model_route_decision.md

Usage:
  python internal/brain/scripts/model_route_decision.py
  python internal/brain/scripts/model_route_decision.py --task-type long_reasoning
  python internal/brain/scripts/model_route_decision.py --task-type product_implementation

Task types:
  simple_report           â†’ local Ollama (fast, private)
  data_entry              â†’ local Ollama or no LLM needed
  long_reasoning          â†’ DeepSeek if configured, else Claude manual or local fallback
  product_implementation  â†’ Claude/Codex manual â€” founder sends mission package
  sensitive_code          â†’ Claude/Codex manual + explicit founder approval required
"""
...
ROUTING: dict[str, dict] = {
    ...
    "data_entry": {
        ...
        "requires_approval": False,
        "guidance": (
            "Data entry tasks use direct CLI scripts â€” no LLM needed.\n"
            "Examples:\n"
    ...
    "long_reasoning": {
        ...
        "privacy": "EXTERNAL â€” only sends context_pack.md contents",
        "requires_approval": True,
        "guidance": (
            "This task needs long-context reasoning across multiple Brain files.\n\n"
            "OPTION A â€” DeepSeek API (if configured):\n"
            "  1. Run: python internal/brain/scripts/check_deepseek_setup.py\n"
            "  2. Run: python internal/brain/scripts/build_context_pack.py\n"
            "  3. Review context_pack.md (never send product code or secrets)\n"
            "  4. Paste context_pack.md into your DeepSeek session manually\n\n"
            "OPTION B â€” Claude manually (no API key needed):\n"
            "  1. Run: python internal/brain/scripts/build_context_pack.py\n"
            "  2. Review context_pack.md\n"
            "  3. Paste into claude.ai manually\n\n"
            "OPTION C â€” Local Ollama fallback (quality may be lower):\n"
            "  Use qwen3:8b with build_context_pack.py output as prompt."
        ),
    },
    "product_implementation": {
        "role": "external_implementation_manual",
        "model_hint": "Claude Code, Codex, or Aider â€” founder sends manually",
        "privacy": "EXTERNAL â€” only v11_mission_package.md is sent",
        "requires_approval": True,
        "guidance": (
    ...
    "sensitive_code": {
        "role": "external_implementation_manual",
        "model_hint": "Claude Code, Codex, or Aider â€” explicit founder approval required",
        "privacy": "EXTERNAL â€” only approved mission package is sent",
        "requires_approval": True,
        "guidance": (
            "SENSITIVE CODE PATH â€” explicit founder approval required before any model.\n\n"
...
def _render_decision(info: dict) -> str:
    ...
    lines: list[str] = [
        "# Vocalype Brain â€” Model Route Decision\n\n",
    ...
    approval_flag = "âš ï¸ YES â€” founder review required" if info["requires_approval"] else "âœ… NO â€” automated OK"

    lines += [
        f"## Task Type\n\n`{info['task_type']}`",
    ]
    if info["resolved"] != info["task_type"]:
        lines.append(f" (resolved from alias â†’ `{info['resolved']}`)")
    lines.append("\n\n")
    ...
    if info.get("resolved") == "long_reasoning":
        ds = "âœ… configured" if info.get("deepseek_configured") else "âŒ DEEPSEEK_API_KEY not set"
...
def main() -> None:
    ...
    if args.task_type is None:
        print("Vocalype Brain â€” Model Routing Table")
        print("=" * 50)
        for tt, info in ROUTING.items():
            print(f"\n  {tt}")
            print(f"    â†’ {info['role']}  |  {info['model_hint']}")
