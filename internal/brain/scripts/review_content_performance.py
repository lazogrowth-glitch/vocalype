"""review_content_performance.py â€” V9 Content Performance Report Generator.
...
Report sections:
  ...
  4. Niche breakdown
  5. Top posts by views (gate: â‰¥1 real post with views)
  6. Hook pattern list
  7. Lessons learned
...
def _build_report(observations: list[dict[str, Any]], now: datetime) -> str:
    ...
    lines += [
        "# Vocalype Brain â€” V9 Content Performance Report",
        "",
    ...
    if not real_obs:
        lines += [
            "> âš ï¸  No real content observations found.",
    ...
    for platform, count in sorted(platform_counts.items(), key=lambda x: -x[1]):
        if platform_views[platform]:
            ...
            avg_str = "â€”"
        note = "" if count >= MIN_POSTS_FOR_RANKING else f"< {MIN_POSTS_FOR_RANKING} posts â€” no ranking yet"
        lines.append(f"| {platform} | {count} | {avg_str} | {note} |")
    lines.append("")

    if len(platform_counts) == 1:
        lines += [
            "> âš ï¸  All posts are from a single platform. "
            "Cross-platform comparison requires posting on â‰¥2 platforms.",
            "",
        ]
    ...
    for ct, count in sorted(type_counts.items(), key=lambda x: -x[1]):
        ...
        if count >= MIN_POSTS_FOR_RANKING and type_views[ct]:
            ...
            rank_note = f"< {MIN_POSTS_FOR_RANKING} posts â€” count only"
    ...
    if posts_with_views:
        ...
        for p in top:
            ...
            likes = str(p.get("likes", "â€”"))
            saves = str(p.get("saves", "â€”"))
    ...
    if hooks:
        ...
        for post_id, platform, hook, views in hooks:
            ...
            views_str = f"{views:,}" if views is not None else "â€”"
    ...
    # --- 8. Coverage gaps ---
    lines += ["## Coverage Gaps â€” Posts Pending Performance Check", ""]
    ...
    if pending:
        ...
        lines += [
            ...
            "Record views and likes 24â€“72h after posting.",
            "",
        ]
    else:
    ...
    lines += [
        ...
        "",
        "> Phase 2 only â€” requires â‰¥10 V9 observations and â‰¥4 weeks of real V8 data.",
        ">",
    ...
    lines += ["## Distribution Backlog â€” Next Experiments", ""]
    next_actions = [
    ...
    if next_actions:
        ...
        lines += [
            ...
            "> No ranking is applied until â‰¥5 posts per format exist (gate G1).",
            "",
        ]
    else:
        lines += [
...
def _append_empty_sections(lines: list[str]) -> None:
    for section in (
        ...
        "Hooks Tested",
        "Lessons Learned",
        "Coverage Gaps â€” Posts Pending Performance Check",
        "V8 Business Metrics Connection",
        "Distribution Backlog â€” Next Experiments",
    ):
...
def main() -> None:
    ...
    print(f"\nWritten: internal/brain/outputs/content_report.md")
    print(divider)
