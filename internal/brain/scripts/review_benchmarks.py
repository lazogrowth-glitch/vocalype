"""review_benchmarks.py â€” V7 Phase 1 Benchmark Reviewer.
...
METRIC_DESCRIPTIONS: dict[str, str] = {
    "total_dictation_latency_ms":       "Total dictation latency (trigger â†’ paste)",
...
NEXT_MEASUREMENT_HINTS: dict[str, str] = {
    "total_dictation_latency_ms":
        "Time trigger â†’ paste complete. Use a stopwatch on 3 warm dictations.",
    "model_load_time_ms":
    ...
    "app_idle_ram_mb":
        "Open Task Manager â†’ Vocalype process â†’ record RSS with no dictation running.",
    "ram_during_transcription_mb":
        "Open Task Manager â†’ Vocalype â†’ record peak RAM while dictating 10s audio.",
    "ram_after_transcription_mb":
...
def _build_report(observations: list[dict[str, Any]], now: datetime) -> str:
    ...
    lines += [f"| Metric | Observations | Baseline ready (â‰¥{MIN_OBSERVATIONS_FOR_BASELINE}) |", "|---|---|---|"]
    for m in PRIORITY_METRICS:
        count = len(by_metric.get(m, []))
        ready = "âœ… Yes" if count >= MIN_OBSERVATIONS_FOR_BASELINE else f"âŒ No ({count}/{MIN_OBSERVATIONS_FOR_BASELINE})"
        ...
        for metric in PRIORITY_METRICS:
            ...
            if recent:
                ...
                for o in recent:
                    ...
                    note = f" â€” {o['notes']}" if o.get("notes") else ""
        ...
        if extra:
            ...
            for m in extra:
                ...
                lines.append(
                    ...
                    f"mean {st.get('mean', '?')} {unit}, "
                    f"range [{st.get('min', '?')}â€“{st.get('max', '?')}]"
                )
            lines.append("")
    ...
    if not observations:
        lines += [
            ...
            "",
            "**Start with M1 â€” Cold start:**",
            "1. Close Vocalype. Reopen it. Start a stopwatch.",
            "2. Record time from launch to first 'ready' state â†’ `model_load_time_ms`",
            "3. Trigger one dictation. Record trigger â†’ paste time â†’ `total_dictation_latency_ms`",
            "4. Open Task Manager. Record Vocalype RAM at idle â†’ `app_idle_ram_mb`",
        ]
    else:
        ...
        if missing:
            ...
            lines += [
                ...
                "Command to record:",
                "```",
                f"python internal/brain/scripts/add_benchmark_observation.py \\",
            ...
            # All priority metrics covered â€” check baseline readiness
            ...
            if not_ready:
                ...
                lines += [
                    f"âœ… All priority metrics have â‰¥{MIN_OBSERVATIONS_FOR_BASELINE} observations.",
                    "",
                    ...
                    "```",
                    "",
                    "> lock_benchmark_baseline.py is a V7 Phase 2 script â€” not yet built.",
                ]
    ...
    lines += [
        ...
        f"- â‰¥{MIN_OBSERVATIONS_FOR_BASELINE} observations exist for every priority metric",
...
def main() -> None:
    ...
    print(f"Baseline ready     : {'YES' if baseline_ready else 'NO â€” collect more observations'}")
    print(f"\nWritten: internal/brain/outputs/benchmark_report.md")
    print(divider)
